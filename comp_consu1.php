...
if (is_ajax())
{
  ...
  switch ($tipo)
  {
    case '1':
      if ($sup_usuario == "1")
      {
        $pregunta = "SELECT *, (SELECT nombre FROM cx_ctr_gas WHERE cx_ctr_gas.codigo=cx_com_ing.concepto) AS concepto1 FROM cx_com_ing WHERE $complemento";
      }
      else
      {
        $pregunta = "SELECT *, (SELECT nombre FROM cx_ctr_gas WHERE cx_ctr_gas.codigo=cx_com_ing.concepto) AS concepto1 FROM cx_com_ing WHERE $complemento AND unidad='$uni_usuario'";
      }
      break;
    case '2':
      if ($sup_usuario == "1")
      {
        $pregunta = "SELECT *, (SELECT nombre FROM cx_ctr_gas WHERE cx_ctr_gas.codigo=cx_com_egr.concepto) AS concepto1 FROM cx_com_egr WHERE $complemento";
      }
      else
      {
        $pregunta = "SELECT *, (SELECT nombre FROM cx_ctr_gas WHERE cx_ctr_gas.codigo=cx_com_egr.concepto) AS concepto1 FROM cx_com_egr WHERE $complemento AND unidad='$uni_usuario'";
      }
      break;
    case '3':
      $pregunta = "SELECT * FROM cv_lib_ban WHERE $complemento";
      if ($sup_usuario == "1")
      {
      }
      else
    ...
    fclose($file);
      ...
      while ($i < $row = odbc_fetch_array($sql))
      {
        ...
        if ($tipo1 == "1")
        {
          ...
          $v_unidad = $c_sigla;
					$pregunta4 = "SELECT n_unidad, n_dependencia FROM cv_unidades WHERE subdependencia='$c_subdepen'";
					$sql4 = odbc_exec($conexion,$pregunta4);
					$n_unidad = trim(odbc_result($sql4,1));
					$n_depen = trim(odbc_result($sql4,2));
          ...
          $saldo = $v_saldo;
          $nrecurso1 = "";
          $valores .= $n_unidad."|".$n_depen."|".$v_unidad."|".$v_unidad."|".$fecha."|".$comprobante."|".$n_tipo."|".$v_ingreso."|".$v_egreso."|".$v_saldo."|".$concepto."|".$soporte1."|".$n_recurso."|".$origen."|".$periodo1."|".$unidad2."|".$n_cuenta."|".$nrecurso1."|#";
        }
        else
        {
          $n_tipo = "EGRESO";
          $pregunta2 = "SELECT tp_gas, soporte, num_sopo, datos, concepto, recurso, periodo, unidad1, cuenta, autoriza, num_auto FROM cx_com_egr WHERE egreso='$comprobante' AND unidad='$unidad' AND periodo='$periodo' AND ano='$ano'";
          $sql2 = odbc_exec($conexion,$pregunta2);
          ...
          $nautoriza = odbc_result($sql2,10);
          $solicitud = odbc_result($sql2,11);
          ...
          for ($h=0; $h<$contador; $h++)
          {
            ...
            $c_fecha = trim(odbc_result($sql3,43));
            $actual = date('Y-m-d');
            if ($c_fecha == "")
            {
            }
            ...
            if ($tot3 == "0")
            {
              $pregunta3 = "SELECT * FROM cx_org_sub WHERE sigla1='$v_unidad'";
              $sql3 = odbc_exec($conexion,$pregunta3);
            }
            $c_unidad = trim(odbc_result($sql3,1));
            $c_depen = trim(odbc_result($sql3,2));
            ...
            if ($nautoriza == "2")
            {
              $pregunta11 = "SELECT recurso FROM cx_pla_inv WHERE conse IN ($solicitud) AND ano='$ano'";
              $sql11 = odbc_exec($conexion,$pregunta11);
              $nrecurso =odbc_result($sql11,1);
              $pregunta12 = "SELECT nombre FROM cx_ctr_rec WHERE codigo='$nrecurso'";
              $sql12 = odbc_exec($conexion,$pregunta12);
              $tot12 = odbc_num_rows($sql12);
              if ($tot12 == "0")
              {
                $nrecurso1 = "N/A";
              }
            ...
            $valores .= $n_unidad."|".$n_depen."|".$v_unidad."|".$v_unidad."|".$fecha."|".$comprobante."|".$n_tipo."|".$v_ingreso."|".$v_egreso."|".$v_saldo."|".$nom_concepto."|".$soporte1."|".$n_recurso."|".$origen."|".$periodo1."|".$unidad2."|".$n_cuenta."|".$nrecurso1."|#";
          }
          $v_egreso = 0; 
        }
      }
  ...
  else
  {
    if ($total>0)
    {
      ...
      while ($i < $row = odbc_fetch_array($sql))
      {
        ...
        $salida->rows[$i]['hora'] = substr($row["fecha"],11,12);
        ...
        $salida->rows[$i]['concepto'] = trim(utf8_encode($row['concepto1']));
...
// 26/04/2024 - Ajuste alltrim en nombre sigla
// 26/06/2024 - Ajuste consulta copcepto y cambio de fecha ingresos
// 04/07/2024 - Ajuste de columnas excel con actualizacion vista cv_unidades
// 16/07/2024 - Ajuste consulta por cambio de unidad en egresos si no se encuentra en la vista cv_unidades
// 29/10/2024 - Ajuste # en campo origen
// 06/11/2024 - Ajuste excel desde superusuario
