...
if (is_ajax())
{
    ...
    if ($sup_usuario == "1")
    {
        $pregunta = "SELECT * FROM cx_act_cen WHERE 1=1";
    }
    else
    {
        $pregunta = "SELECT * FROM cx_act_cen WHERE unidad='$uni_usuario' AND ((usuario='$usu_usuario') OR (usuario='$log_usuario'))";
    }
    ...
    if ($total>0)
    {
        ...
        while ($i < $row = odbc_fetch_array($sql))
        {
            ...
            $ano = $row['ano1'];
            $pregunta1 = "SELECT unidad, ordop, fragmenta, codigo, valor FROM cx_reg_rec WHERE conse='$registro' AND ano='$ano'";
            $sql1 = odbc_exec($conexion,$pregunta1);
            $unidad1 = odbc_result($sql1,1);
            $ordop = trim(utf8_encode(odbc_result($sql1,2)));
            $fragmenta = trim(utf8_encode(odbc_result($sql1,3)));
            $codigo = trim(odbc_result($sql1,4));
            $solicitado = trim(odbc_result($sql1,5));
            ...
            $salida->rows[$i]['fragmenta'] = $fragmenta;
            $salida->rows[$i]['solicitado'] = $solicitado;
...
// 02/05/2024 - Inclusion columna valor solicitado por unidad
?>