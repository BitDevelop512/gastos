  ...
  for ($j=0; $j<count($longitud); $j++)
  {
    ...
    if ($total > 0)
    {
      ...
      for ($l=0; $l<count($longitud1); $l++)
      {
        ...
        $pregunta1 = "SELECT gasto, SUM(valor1) AS valor, (SELECT nombre FROM cx_ctr_pag WHERE cx_ctr_pag.codigo=cx_rel_dis.gasto) AS n_gasto FROM cx_rel_dis WHERE EXISTS(SELECT * FROM cx_rel_gas WHERE cx_rel_gas.conse=cx_rel_dis.conse1 AND cx_rel_gas.consecu=cx_rel_dis.consecu AND cx_rel_gas.unidad='$valor') AND conse1 IN ($conses) AND consecu IN ($consecus) AND gasto='$valor1' GROUP BY gasto";
