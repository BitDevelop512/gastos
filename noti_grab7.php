...
if (is_ajax())
{
	...
	if (($paso == "4") or ($paso == "8") or ($paso == "9"))
	{
		...
		$v1 = trim(odbc_result($cur1,1));
		$v2 = "CDO_".$v1;
		$query0 = "SELECT usuario FROM cx_usu_web WHERE (usuario LIKE '%$v2%') OR (login LIKE '%$v2%')";
	}
  ...
  $preg0 = "INSERT INTO cx_pla_est (conse, solicitud, ano, usuario, unidad, estado, usuario1) VALUES ($query_c0, '$valor2', '$ano', '$usu_usuario', '$uni_usuario', '', '$valor')";
...
// 26/04/2024 - Registro en estados de solicitud desde inicio de solicitud
?>