      ...
      <div class="col col-lg-12 col-sm-12 col-md-12 col-xs-12">
        <div id="soportes">
          ...
          <div>
            ...
            <form name="formu" method="post">
              <div class="row">
                <div class="col col-lg-3 col-sm-3 col-md-3 col-xs-3">
                  <label><font face="Verdana" size="2">Tipo de Reporte</font></label>
                  <select name="reporte" id="reporte" class="form-control select2" tabindex="1">
                    <option value="1">CONCEPTOS DEL GASTO</option>
                ...
                <div id="lbl1">
                  <div class="col col-lg-3 col-sm-3 col-md-3 col-xs-3">
                    ...
                    <select name="tipo" id="tipo" class="form-control select2" tabindex="1">
                      <option value="1">GASTOS SOLICITADOS</option>
                      <option value="2">GASTOS EJECUTADOS</option>
                      <option value="3">SOLICITADOS VS EJECUTADOS</option>
                    </select>
                  </div>
                  <div class="col col-lg-3 col-sm-3 col-md-3 col-xs-3">
                    <label><font face="Verdana" size="2">Concepto del Gasto</font></label>
                    <?php
                    $menu1_1 = odbc_exec($conexion,"SELECT nombre, codigo FROM cx_ctr_pag WHERE tipo NOT IN ('R','X') ORDER BY codigo");
                    $menu1 = "<select name='concepto' id='concepto' class='form-control select2' multiple='multiple' data-placeholder='&nbsp;Seleccione uno o mas conceptos'>";
                    $i = 1;
            ...
            <form name="formu_excel" id="formu_excel" action="conceptos_x.php" target="_blank" method="post">
              <input type="hidden" name="paso_excel" id="paso_excel" class="form-control" readonly="readonly">
              <input type="hidden" name="paso_reporte" id="paso_reporte" class="form-control" readonly="readonly">
            </form>
            <form name="formu_excel1" id="formu_excel1" action="misiones_x.php" target="_blank" method="post">
              <input type="hidden" name="paso_excel1" id="paso_excel1" class="form-control" readonly="readonly">
            </form>
            <form name="formu_excel2" id="formu_excel2" action="conceptos1_x.php" target="_blank" method="post">
              <input type="hidden" name="paso_excel2" id="paso_excel2" class="form-control" readonly="readonly">
            </form>
          </div>
          <div id="dialogo"></div>
        </div>
...
$(document).ready(function () {
  ...
  $("#reporte").focus();
  $("#reporte").change(valida);
  valida();
});
function valida()
{ 
  var consulta = $("#reporte").val();
  if (consulta == "1")
  {
    $("#lbl1").show();
  }
...
function consultar()
{
  ...
  var consulta = $("#reporte").val();
  var tipo = $("#tipo").val();
  var concepto = $("#concepto").select2('val');
  ...
  else
  {
    if (consulta == "1")
    {
      var url = "repo_var1.php";
    }
    else
    {
      var url = "repo_var2.php"; 
    }
    $.ajax({
      type: "POST",
      datatype: "json",
      ...
      success: function (data)
      {
        ...
        if (consulta == "1")
        {
        	if (tipo == "3")
        	{
	        	$("#paso_excel2").val(registros.valores);
	        	excel2();
        	}
        	else
        	{
		        $("#paso_excel").val(registros.valores);
		        $("#paso_reporte").val(tipo);
		        excel();
					}
				}
				else
				{
	        $("#paso_excel1").val(registros.valores);
	        excel1();
				}
      }
    });
...
// 30/07/2024 - Ajuste envio datos a excel de reporte versus
