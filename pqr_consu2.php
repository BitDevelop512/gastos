    ...
    if ($tipo == "1")
    {
        ...
        if ($total>0)
        {
            ...
            while ($i < $row = odbc_fetch_array($sql))
            {
                ...
                $n_concepto = str_replace("#", "No", $concepto);
                $solucion = trim(utf8_encode($row['solucion']));
                $n_solucion = str_replace("#", "No", $solucion);
                ...
                $valores .= $row['conse']."|".substr($row["fecha"],0,10)."|".trim($row['usuario'])."|".$row['ano']."|".$n_estado."|".$n_unidad."|".$n_tipo."|".$n_modulo."|".$n_submodulo."|".$n_concepto."|".$n_solucion."|#";
                $i++;
            }
        	$salida->salida = "1";
          	$salida->total = $total;
...
// 11/06/2024 - Ajuste campo concepto con caracter #
?>