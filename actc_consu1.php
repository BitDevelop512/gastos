...
if (is_ajax())
{
    ...
    $ano1 = $_POST['ano1'];
    $pregunta = "SELECT conse, firmas, sintesis, concepto, informacion, observaciones, elaboro, neutralizados, totaln, material, totalm, totala, valor, directiva, registro, ano1, acta, constancia, folio, valoracion, totali, reviso, fec_act, impacto, aprobacion, cargo, cargo1, anticipo FROM cx_act_cen WHERE conse='$conse' AND ano='$ano' AND registro='$registro' AND ano1='$ano1'";
    ...
    $material1 = trim(utf8_encode(odbc_result($sql,10)));
    ...
    $cargo = trim(utf8_encode(odbc_result($sql,26)));
    $cargo1 = trim(utf8_encode(odbc_result($sql,27)));
    $anticipo = trim(odbc_result($sql,28));
    if (($anticipo == "") or ($anticipo == "NaN"))
    {
        $anticipo = "0.00";
    }
...
// 04/02/2025 - Ajuste codificacion
?>