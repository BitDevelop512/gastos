...
if (is_ajax())
{
      ...
      case '2':
        ...
        while ($i < $row = odbc_fetch_array($sql))
        {
          ...
          if ($tipo == "2")
          {
            if ($tipo2 == "2")
            {
              $pregunta2 = "SELECT conse, ano FROM cx_pla_inv WHERE unidad='$unidad' AND ((CONVERT(datetime,fecha,102) BETWEEN CONVERT(datetime,'$fecha1',102) AND (CONVERT(datetime,'$fecha2',102)+1)) OR periodo='$mes') AND EXISTS (SELECT * FROM cx_pla_gad WHERE cx_pla_gad.conse1=cx_pla_inv.conse AND cx_pla_gad.ano=cx_pla_inv.ano AND cx_pla_gad.gasto='$codigo') ORDER BY fecha DESC";
            }
            else
            {
              $pregunta2 = "SELECT conse, ano FROM cx_pla_inv WHERE unidad='$unidad' AND ((CONVERT(datetime,fecha,102) BETWEEN CONVERT(datetime,'$fecha1',102) AND (CONVERT(datetime,'$fecha2',102)+1)) OR periodo='$mes1') AND EXISTS (SELECT * FROM cx_pla_gad WHERE cx_pla_gad.conse1=cx_pla_inv.conse AND cx_pla_gad.ano=cx_pla_inv.ano AND cx_pla_gad.gasto='$codigo') ORDER BY fecha DESC";
            }
  ...
  if ($tipo == "2")
  {
    ...
    if ($tipo2 == "2")
    {
      $pregunta1 = "SELECT conse FROM cx_pla_inv WHERE periodo='$mes' AND ano='$ano' AND usuario='$usu_usuario' AND (estado!='' AND estado!='X') ORDER BY conse";
    }
    else
    {
      $pregunta1 = "SELECT conse FROM cx_pla_inv WHERE periodo='$mes1' AND ano='$ano' AND usuario='$usu_usuario' AND (estado!='' AND estado!='X') ORDER BY conse";
    }
