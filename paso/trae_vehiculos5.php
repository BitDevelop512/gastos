...
if (is_ajax())
{
	...
	$query1 = "SELECT codigo, marca, descripcion, valor, iva, total, (SELECT SUBSTRING(descripcion, 0, 50)) AS descripcion1 FROM cx_ctr_lla WHERE tipo='$tipo' AND marca LIKE '%$marca%' AND estado='' ORDER BY descripcion1";
	...
	if ($t_cur1 > 0)
	{
		...
		while($i<$row=odbc_fetch_array($cur1))
		{
			...
			$marca = trim(odbc_result($cur1,2));
			$descripcion = trim(utf8_encode($row['descripcion']));
			$valor = odbc_result($cur1,4);
			$iva = odbc_result($cur1,5);
			$total = odbc_result($cur1,6);
			$datos .= $codigo."|".$marca."|".$descripcion."|".$valor."|".$iva."|".$total."|#";
			$i++;
		}
	}
	$salida->datos = $datos;
...
// 19/12/2023 - Llantas x contratos
