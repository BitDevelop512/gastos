...
if (is_ajax())
{
	...
	$file = fopen("log_veri_contrator.txt", "a");
	...
	$cur = odbc_exec($conexion,"SELECT isnull(max(conse),0)+1 AS conse FROM cx_tra_rtm");
	$consecu = odbc_result($cur,1);
	$busca = "SELECT conse FROM cx_tra_rtm WHERE fecha='$fecha' AND placa='$placa'";
	...
	if ($borrar == "0")
	{
		$graba = "INSERT INTO cx_tra_rtm (conse, fecha, usuario, unidad, placa, kilometraje, valores, total, total1, contrato, contrato1, ano, elaboro, alea) VALUES ('$consecu', '$fecha', '$usu_usuario', '$uni_usuario', '$placa', '$kilometraje', '$valores', '$precio', '$precio1', '$contrato', '$contrato1', '$ano', '$elaboro', '$alea')";
	...
	{
		$borra = "DELETE FROM cx_tra_rtm WHERE fecha='$fecha' AND placa='$placa'";
	...
	// Log
	$fec_log = date("d/m/Y H:i:s a");
	$file = fopen("log_movi_transportes_rtm.txt", "a");
