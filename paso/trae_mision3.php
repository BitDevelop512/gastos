...
if (is_ajax())
{
	...
	$ordop1 = trim($ordop1);
	$conse = $_POST['conse'];
	$query = "SELECT conse, misiones, estado, n_misiones, tipo FROM cx_pla_inv WHERE unidad='$uni_usuario' AND compania='$tip_usuario' AND ordop='$ordop' AND n_ordop='$ordop1' AND estado IN ('D','F','G','H','W') AND periodo IN ('$periodo','$periodo1') AND ano='$ano' ORDER BY conse";
	...
	while($i<$row=odbc_fetch_array($cur))
	{
		...
		for ($m=0;$m<count($misiones1)-1;++$m)
		{
			...
			if ($tipo == "1")
			{
				$query0 = "SELECT conse, mision, interno FROM cx_pla_gas WHERE conse1='$conse' AND ano='$ano' AND unidad='$uni_usuario' AND mision='$misiones2' AND autoriza='1' AND informe!='0' ORDER BY conse";
			}
			else
			{
				$query0 = "SELECT conse, mision, interno FROM cx_pla_gas WHERE conse1='$conse' AND ano='$ano' AND unidad='$uni_usuario' AND mision='$misiones2' AND autoriza='0' AND informe='0' ORDER BY conse";
			}
			...
			if ($total0 > 0)
			{
				...
				$internos .= odbc_result($cur0,3).",";
				$misiones3 .=  $misiones2."|"; 
				$querys .= "SELECT conse FROM cx_inf_eje WHERE unidad='$uni_usuario' AND ordop='$ordop4' AND n_ordop='$ordop3' AND numero='$v_interno' AND consecu='$conse' AND ano='$ano'"."#";
			}
			else
			{
				$internos .= "0,";
