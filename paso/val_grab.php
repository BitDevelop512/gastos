...
if (is_ajax())
{
	...
	if ($adm_usuario == "6")
	{
		...
		$graba = "INSERT INTO cx_pla_cen (conse, usuario, unidad, periodo, ano, saldo, gastos, pagos, recompensas, gastos1, pagos1, recompensas1) VALUES ('$consecu', '$usu_usuario', '$uni_usuario', '$periodo', '$ano', '$saldo', '$gastos1', '$pagos1', '$recompensas1', '$gastos2', '$pagos2', '$recompensas2')";
		...
		$mensaje = "SE HA REGISTRADO SALDO EN BANCOS DEL CONSOLIDADO PLAN DE NECESIDADES UNIDAD CENTRALIZADORA, SE SOLICITA APROBACION.";		
	...
	if (($adm_usuario == "7") or ($adm_usuario == "8"))
	{
		if (($nun_usuario == "1") or ($nun_usuario == "2") or ($nun_usuario == "3"))
        {
			...
			{
				$val_est = "C";
				$graba1 = "UPDATE cx_pla_cen SET revisa='$con_usuario' WHERE unidad='$uni_usuario' AND periodo='$periodo' AND ano='$ano'";
			}
			else
			{
				$val_est = "D";
				$graba1 = "UPDATE cx_pla_cen SET ordena='$con_usuario' WHERE unidad='$uni_usuario' AND periodo='$periodo' AND ano='$ano'";
			}
		}
		else
		{
		...
		if ($adm_usuario == "7")
		{
			...
			$mensaje = "SE HA CONSOLIDADO PLAN DE NECESIDADES UNIDAD CENTRALIZADORA, SE SOLICITA APROBACION.";
		}
		else
		{
			$query2 = "SELECT usuario FROM cx_usu_web WHERE unidad='$uni_usuario' AND admin='9'";
			$mensaje = "SE HA CONSOLIDADO PLAN DE NECESIDADES UNIDAD CENTRALIZADORA, SE SOLICITA VISTO BUENO DE LA ORDENACION.";
		}	    
	   	$cur2 = odbc_exec($conexion,$query2);
	   	$usuario1 = odbc_result($cur2,1);
		$query2 = "INSERT INTO cx_notifica (conse, usuario, unidad, usuario1, unidad1, mensaje, tipo, visto) VALUES ('$consecu1', '$usu_usuario', '$uni_usuario', '$usuario1', '$uni_usuario', '$mensaje', 'S', '1')";
	...
	if ($adm_usuario == "13")
	{
		$graba1 = "UPDATE cx_pla_cen SET visto='$con_usuario' WHERE unidad='$uni_usuario' AND periodo='$periodo' AND ano='$ano'";
		odbc_exec($conexion, $graba1);
		$conse = "1";
		...
		$mensaje = "SE HA CONSOLIDADO PLAN DE NECESIDADES UNIDAD CENTRALIZADORA: ".trim($siglas);
