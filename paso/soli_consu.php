...
if (is_ajax())
{
    ...
    $fecha2 = $_POST['fecha2'];
    $sustituye = array ( "'" => "" );
    $pregunta = "SELECT * FROM cx_sol_aut WHERE 1=1";
    ...
    if ($total>0)
    {
        ...
        {
            $unidad = $row['unidad'];
            $planes = trim(decrypt1($row['planes'], $llave));
            $planes = strtr(trim($planes), $sustituye);
            $planes = str_replace(",", ", ", $planes);
...
// 17/10/2023 - Revision y ajuste consulta
