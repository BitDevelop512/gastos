...
if(isset($_POST['cargar']))
{
  ...
  if (move_uploaded_file($_FILES['archivo']['tmp_name'],"excel/" . $archivo1))
  {
    ...
    $actu = "UPDATE cx_ctr_rtm SET estado='X' WHERE estado='' AND tipo='$tipo'";
    ...
    for ($row = 1; $row <= $lastRow; $row++)
    {
      ...
      else
      {
        ...
        $a = trim($worksheet->getCell('B'.$row)->getValue());                                 // Descripcion
        $a = iconv("UTF-8", "ISO-8859-1", $a);
        echo utf8_encode($a)."</font></td>";

        echo "<td width='10%' align='right'><font face='Verdana' size='2'>";
        $b = trim($worksheet->getCell('C'.$row)->getValue());                                 // Valor Unitario
        echo number_format($b, 2)."</font></td>";

        echo "<td width='10%' align='right'><font face='Verdana' size='2'>";
        $c = trim($worksheet->getCell('D'.$row)->getValue());                                 // IVA
        echo number_format($c, 2)."</font></td>";

        echo "<td width='10%' align='right'><font face='Verdana' size='2'>";
        $d = trim($worksheet->getCell('E'.$row)->getValue());                                 // Valor Total
        ...
        if ($row > 1)
        {
          ...
          else
          {
            ...
            $sql1 = odbc_exec($conexion,"SELECT isnull(max(codigo),0)+1 AS conse FROM cx_ctr_rtm");
            $consecu = odbc_result($sql1,1);
            $graba = "INSERT INTO cx_ctr_rtm (codigo, descripcion, tipo, valor, iva, total, usuario, importa) VALUES ('$consecu', '$a', '$tipo', '$b', '$c', '$d', '$usu_usuario', '$archivo2')";
            // Se graba log
            if (!odbc_exec($conexion, $graba))
            {
              $fec_log = date("d/m/Y H:i:s a");
              $file = fopen("log_importa_rtm_err.txt", "a");
            ...
            {
              $fec_log = date("d/m/Y H:i:s a");
              $file = fopen("log_importa_rtm_ok.txt", "a");
              fwrite($file, $fec_log." # ".$graba." # ".PHP_EOL);
              fclose($file);
            }
          }
