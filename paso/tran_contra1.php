...
if (is_ajax())
{
    ...
    if ($total>0)
    {
        $pregunta1 = "SELECT ISNULL(MAX(kilometraje), 0) AS kilometraje FROM cx_tra_moc WHERE placa='$placa' AND convert(datetime,fecha,102) BETWEEN convert(datetime,'$fecha1',102) AND (convert(datetime,'$fecha2',102)+1)";
