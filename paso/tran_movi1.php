...
if (is_ajax())
{
  ...
  if ($total>0)
  {
    $pregunta1 = "SELECT ISNULL(MAX(kilometraje), 0) AS kilometraje FROM cx_tra_mov WHERE placa='$placa' AND convert(datetime,fecha,102) BETWEEN convert(datetime,'$fecha1',102) AND (convert(datetime,'$fecha2',102)+1)";
    ...
    if (($kilometros == "0") or ($kilometros == "0.00"))
    {
      ...
      $pregunta1 = "SELECT ISNULL(MAX(kilometraje), 0) AS kilometraje FROM cx_tra_mov WHERE placa='$placa' AND convert(datetime,fecha,102) BETWEEN convert(datetime,'$fecha3',102) AND (convert(datetime,'$fecha2',102)+1)";
      $sql1 = odbc_exec($conexion,$pregunta1);
      $kilometros = odbc_result($sql1,1);
      $kilometros = floatval($kilometros);
      if (($kilometros == "0") or ($kilometros == "0.00"))
