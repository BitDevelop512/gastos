...
if (is_ajax())
{
    ...
    $query = "SELECT conse, actual, unidad FROM cx_pla_inv WHERE periodo='$mes' AND ano='$ano' AND tipo='2' AND tipo1='$tipo' AND estado='W' AND aprueba='0' AND unidad IN ($numeros) ORDER BY conse";
    $cur = odbc_exec($conexion, $query);
    $datos = "";
    while($i<$row=odbc_fetch_array($cur))
    {
