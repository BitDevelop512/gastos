...
if (is_ajax())
{
    ...
    $ano1 = $_POST['ano1'];
    $pregunta = "SELECT conse, firmas, sintesis, concepto, recomendaciones, observaciones, elaboro, neutralizados, totaln, material, totalm, totala, valor, directiva, registro, ano1, acta, constancia, folio, valoracion, reviso, impacto, informacion, totali FROM cx_act_reg WHERE conse='$conse' AND ano='$ano' AND registro='$registro' AND ano1='$ano1'";
    ...
    $reviso = trim(utf8_encode(odbc_result($sql,21)));
    $impacto = trim(utf8_encode(odbc_result($sql,22)));
    $informacion = trim(utf8_encode(odbc_result($sql,23)));
    $totali = trim(odbc_result($sql,24));
    $pregunta1 = "SELECT salario FROM cx_ctr_ano WHERE ano='$ano1'";
    $sql1 = odbc_exec($conexion,$pregunta1);
    $salario = odbc_result($sql1,1);
    $pregunta2 = "SELECT usuario3, codigo FROM cx_reg_rec WHERE conse='$registro' AND ano='$ano1'";
    $sql2 = odbc_exec($conexion,$pregunta2);
    $usuario = trim(odbc_result($sql2,1));
    $codigo = trim(odbc_result($sql2,2));
