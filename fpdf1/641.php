...
*/
	...
	else
	{
		...
		class PDF extends PDF_Rotate
		{
			function Header()
			{
				...
				else
				{
					...
					$this->RotatedText(57,200,$sig_usuario,35);
				}
		...
		function control_salto_pag($actual1)
		{
			global $pdf;
			$actual1=$actual1+5;
			if ($actual1>=229.00125) $pdf->addpage();
		}
		...
		$var_1 = odbc_result($cur13,1);
		$var_2 = odbc_result($cur13,2);
		$var_uni = trim(odbc_result($cur13,4));
		$tipo_c = odbc_result($cur13,7);
		$especial = trim(odbc_result($cur13,40));
		...
		$var_dep = trim(odbc_result($cur14,1));

		$consulta15 = "select nombre from cx_org_uni where unidad='$var_1'";
		$cur15 = odbc_exec($conexion,$consulta15);
		$var_div = trim(odbc_result($cur15,1));

		if ($tipo1 == "1") $tipo = "GASTOS EN ACTIVIDADES";
		else $tipo = "PAGO DE INFORMACIONES";

		$elaboro = explode("»",trim(odbc_result($cur,45)));   //firma1
		$l_pos = strlen($elaboro[0]);
		if (substr(utf8_decode($elaboro[0]), -1) == "?") $n_elaboro = substr($elaboro[0],0,$l_pos-1);	
		else $n_elaboro = $elaboro[0];	
		$l_pos = strlen($elaboro[1]);
		if (substr(utf8_decode($elaboro[1]), -1) == "?") $c_elaboro = substr($elaboro[1],0,$l_pos-1);	
		else $c_elaboro = $elaboro[1];

		$reviso = explode("»",trim(odbc_result($cur,47)));  //firma3
		$l_pos = strlen($reviso[0]);
		if (substr(utf8_decode($reviso[0]), -1) == "?") $n_reviso = substr($reviso[0],0,$l_pos-1);
		else $n_reviso	= $reviso[0];	
		$l_pos = strlen($reviso[1]);
		if (substr(utf8_decode($reviso[1]), -1) == "?") $c_reviso = substr($reviso[1],0,$l_pos-1);
		else $c_reviso = $reviso[1];	

		$aprobo = explode("»",trim(odbc_result($cur,48)));   //firma4
		$l_pos = strlen($aprobo[0]);
		if (substr(utf8_decode($aprobo[0]), -1) == "?") $n_aprobo = substr($aprobo[0],0,$l_pos-1);	
		else $n_aprobo	= $aprobo[0];	
		$c_aprobo = "FIRMA SOLICITANTE"; 

		$jem = explode("»",trim(odbc_result($cur,50)));   //firma6
		$l_pos = strlen($jem[0]);
		if (substr(utf8_decode($jem[0]), -1) == "?") $n_jem = substr($jem[0],0,$l_pos-1);	
		else $n_jem = $jem[0];	
		$l_pos = strlen($jem[1]);
		if (substr(utf8_decode($jem[1]), -1) == "?") $c_jem = substr($jem[1],0,$l_pos-1);	
		else $c_jem = $jem[1];	

		$cdo = explode("»",trim(odbc_result($cur,51)));
		$l_pos = strlen($cdo[0]);
		if (substr(utf8_decode($cdo[0]), -1) == "?") $n_cdo = substr($cdo[0],0,$l_pos-1);	
		else $c_cdo = $cdo[0];
		$l_pos = strlen($cdo[1]);
		if (substr(utf8_decode($cdo[1]), -1) == "?") $c_cdo = substr($cdo[1],0,$l_pos-1);	
		else $c_cdo = $cdo[1];
		...
		$pdf->Cell(20,5,'Radicado',1,0,'');
		$pdf->Cell(17,5,$consecu,0,1,'C');
		$actual=$pdf->GetY();
		$pdf->RoundedRect(9,$actual,192,5,0,'');
		$pdf->Cell(27,5,'Unidad Solicitante',0,0,'');
		$pdf->Cell(164,5,$var_uni,1,1,'');
		$actual=$pdf->GetY();
		$pdf->RoundedRect(9,$actual,192,5,0,'');
		$pdf->Cell(27,5,'U.O.M:',0,0,'');

		if ($especial != 0)
		{
			$consulta_es = "select * from cx_org_sub where unidad = '".$especial."'";
			$cur_es = odbc_exec($conexion,$consulta_es);
			$var_div = trim(odbc_result($cur_es,4));
		}	//if
		$pdf->Cell(18,5,$var_div,1,0,'');

		$pdf->Cell(17,5,'Brigada:',1,0,'');
		...
		$pdf->Cell(18,5,$var_dep,1,0,'');

		$pdf->Cell(17,5,'Batallón:',1,0,'');
		...
		//if ($var_1 >= 4 and $var_1 <= 17 and ($tipo_c == 6 or $tipo_c == 9)) $var_uni = "    ";
		if ($var_1 >= 4 and $var_1 <= 21 and ($tipo_c == 6 or $tipo_c == 9)) $var_uni = "    ";
		$pdf->Cell(18,5,$var_uni,1,0,'');

		$pdf->Cell(29,5,'Compañía/Área:',1,0,'');
		if ($var_1 == 19 or $var_1 == 21) if ($tipo_c == 7 or $tipo_c == 9) $n_compania = $var_div;
		if ($var_1 == 21 and $tipo_c == 4 and $unic == 2) $n_compania = 'CENOR';
		//if ($var_1 == 19 or $var_1 == 21 and $tipo_c == 8) $n_compania = $var_uni;
		$pdf->Cell(47,5,$n_compania,1,1,'');

		$actual=$pdf->GetY();
		$pdf->RoundedRect(9,$actual,192,5,0,'');
		if ($tipo1 == "1")
		{
			$pdf->Cell(27,5,'ORDOP',0,0,'');
			$pdf->Cell(117,5,utf8_decode($ordop),1,0,'');
			$pdf->Cell(12,5,'MISIÓN',0,0,'');
			$pdf->Cell(35,5,utf8_decode($misiones),1,1,'');
		}   //if
		
		$actual=$pdf->GetY();
		$pdf->RoundedRect(9,$actual,192,5,0,'');
		$pdf->Cell(27,5,'Concepto del Gasto',0,0,'');
		$pdf->Cell(164,5,$tipo,1,1,'');
		$actual=$pdf->GetY();
		...
		if ($tipo1 == "1")
		{
			$consulta5 = "select * from cx_pla_gas where conse1='$conse' and ano = '$ano'";
		...
		else
		{
			...
			while($k<$row=odbc_fetch_array($cur6))
			{
				...
				$vap2 = "Factor de Amenaza: ".$n_factor."   Estructura: ".$n_estructura."   OMAVE - OMINA - OMIRE: ".$n_oms."\n";
				$vap3 = "Fecha Sumnistro Información: ".$fes_fuen."\n";
				...
				else $vap6 = "Condujo Resultado: SI   Reporte Radiograma: ".$rad_fuen."   Fecha: ".$fer_fuen."\n\n";
		...
		if ($t_bienes == "") $pdf->Multicell(192,20,$t_bienes,'','L');
		...
		$pdf->Cell(1,5,'',0,1,'C');					
		if ($t_justif == "") $pdf->Multicell(192,20,$t_justif,'','L');
		...
		$pdf->RoundedRect(9,$actual+5,139,5,0,'');		
		$pdf->RoundedRect(9,$actual,192,5,0,'');		
		$pdf->Cell(138,5,'Valor Autorizado:',0,0,'');
		$pdf->Cell(53,5,wims_currency($tvalora),1,1,'R');
		...
		//$pdf->Image($file,140,$actual,$w,26);
		$pdf->Ln(4);
		...
		$pdf->Cell(95,5,$n_jem,0,0,'C');
		$pdf->Cell(95,5,$n_cdo,0,1,'C');
		$pdf->Cell(95,5,$c_jem,0,0,'C');
		$pdf->Cell(95,5,$c_cdo,0,1,'C');
		$pdf->Cell(190,4,$linea,0,1,'C');

		//$pdf->Ln(6);
		$pdf->SetFont('Arial','',7);
		$pdf->Cell(95,4,$firma_ordenador,0,0,'C');
		$pdf->Cell(95,4,$firma_comandante,0,1,'C');
		$pdf->Cell(95,4,$cargo_ordenador,0,0,'C');
		$pdf->Cell(95,4,$cargo_comandante,0,1,'C');
		$pdf->SetFont('Arial','',8);
		//$pdf->Ln(2);
		$pdf->Cell(190,7,'Anexos:',0,1,'');
		...
		$pdf->Cell(85,4,$n_elaboro,0,0,'');
		$pdf->Cell(15,4,'Revisó:',0,0,'');
		$pdf->Cell(74,4,$n_reviso,0,1,'');
		$pdf->Cell(190,4,$linea,0,1,'C');
