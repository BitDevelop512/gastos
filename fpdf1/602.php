...
/* 602.php
   ...
   (pág 147 - "Directiva Permanente No. 00000 de 20.PDF")
*/
...
else
{
	...
	$reemplazar = array("", "", "", "", "", "" , "");
	$n_meses = array('Eenero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre');
	$linea = str_repeat("_",108);   //109
	...
	for ($f=0;$f<=$cn;$f++)
	{
		...
		$pdf->Multicell(60,$aa,'$'.trim(substr($neutral1[2],0,-1)),0,'R');
		$pdf->SetXY($x+140,$y);
		$pdf->RoundedRect(155,$y,45,$aa,0,'');		
		$pdf->Multicell(60,$aa,'$'.trim(substr($neutral1[4],0,-1)),0,'R');
		$x = 0;
		$y = $pdf->GetY();		
	}   //for
	...
	{	
		...
		$actual=$pdf->GetY();
		$pdf->RoundedRect(10,$actual,80,$aa,0,'D');
		$x = $pdf->Getx();
		$y = $pdf->Gety();
		$pdf->SetXY($x,$y);		
		$pdf->Multicell(80,3,"\n".$elemento1,0,'J');		
		$pdf->SetXY($x+80,$y);
		$pdf->Multicell(19,$aa,number_format(substr($material1[1],0,-1),0),1,'C');		
		$pdf->SetXY($x+99,$y);		
		$pdf->Multicell(44,$aa,'$'.number_format(trim($material1[3]),2),1,'R');   //revisar
		$pdf->SetXY($x+143,$y);
		$pdf->Multicell(14,$aa,trim(substr($material1[4],0,-1)),1,'C');
		$pdf->SetXY($x+157,$y);
		$pdf->Multicell(33,$aa,'$'.number_format(trim($material1[6]),2),1,'R');
		control_salto_pag($pdf->GetY());
	...
	$actual = $pdf->GetY();
	$pdf->Cell(95,4,'Elaboró:    '.utf8_encode($_SESSION["nom_usuario"]),0,0,'');
	$pdf->Cell(95,4,'Revisó:    '.$n_reviso,0,1,'');
	$pdf->Cell(190,3,$linea2,0,1,'C');

	$nom_pdf="../fpdf/pdf/Actas/ActaComReg_".trim($sig_usuario)."_".$conse."_".$ano.".pdf";
