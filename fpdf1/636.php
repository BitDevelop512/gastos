...
/* 636.php (plan_necesidades.php)
   FO-JEMPP-CEDE2-636- Plan de Necesidades de Gastos Reservados para la Intelligencia y Contrainteligencia.
...
else
{
	...
	function control_salto_pag($actual1)
	{
		global $pdf;
		$actual1=$actual1+5;
		if ($actual1>=232.00125) $pdf->addpage();
	} //control_salto_pag

	function titulo_c($actual1, $y1, $titulos11, $unidad1, $flag, $item)
	{
		require('../conf.php');		
		global $pdf;
		...
		while($f<=$nreg=odbc_fetch_array($cur_unidad))
		{
			...
			$query1 = "select unidad from cx_org_sub where subdependencia='$subdepen[$f]'";
			...
			//Se consulta en la tabla val_aut
			if ($esp <> 0) $consulta_subdepend = "select gastos, pagos, total from cx_val_aut where estado in ('V','I','G') and unidad = '".$subdepen[$f]."' and periodo = '".$_GET['periodo']."' and ano = '".$_GET['ano']."'";  // and exists(select * from cx_inf_pla where unidad = '".$centra."' and periodo = '".$_GET['periodo']."' and ano = '".$_GET['ano']."' and aprueba>0)";
			...
			if($nreg_subdepend <> 0)
			{
				...
				$pdf->Cell(13,4,$item.$pos,0,0,'C');			
				$pdf->Cell(82,4,odbc_result($cur_unidad,4),1,0,'');
				$gastosiyc = odbc_result($cur_subdepend,1);
				$pagoasinf = odbc_result($cur_subdepend,2);
				$pdf->Cell(48,4,'$'.number_format($gastosiyc,2),1,0,'R');
				$pdf->Cell(48,4,'$'.number_format($pagoasinf,2),1,1,'R');
	...
	$cur1 = odbc_exec($conexion,$consulta1);
	$ciudad = trim(odbc_result($cur1,1));
	...
	$ciudad1 = strtr($ciudad, $sustituye);
	$ciudadyfecha = $ciudad1."   -   ".$fecha;    //date("Y-m-d");
	$pdf->Cell(25,4,'Lugar y Fecha,',0,0,'');
	$pdf->Cell(166,4,$ciudadyfecha,B,1,'L');
	$pdf->Ln(2);	
	...
	{
		$actual=$pdf->GetY();
		$suman_recursos = $suman_recursos + titulo_c($actual, $y, $titulos1[$y], $unidades1[$y], '1', $item1[$y]);
		control_salto_pag($pdf->GetY());
	} //for
	...
	{
		$actual=$pdf->GetY();
		$suman_recursos = $suman_recursos + titulo_c($actual, $y, $titulos2[$y], $unidades2[$y], '2', $item2[$y]);
		control_salto_pag($pdf->GetY());
	} //for
	...
	{
		$actual=$pdf->GetY();
		$suman_recursos = $suman_recursos + titulo_c($actual, $y, $titulos3[$y], $unidades3[$y], '3', $item3[$y]);	
		control_salto_pag($pdf->GetY());		
	} //for
	...
	$consulta_usuweb = "select * from cx_usu_web where nombre = '".$n_aprueba."'"; 
	$cur_usuweb = odbc_exec($conexion,$consulta_usuweb);
	$c_aprueba = strtr(trim(odbc_result($cur_usuweb,18)), $sustituye);

	//Busca imagen de la firma Aprobo
	//$n_aprueba = "JAIME ALBERTO MORALES (CX)";
	$consulta_fr = "select firma, usuario FROM cx_usu_web WHERE nombre='$n_aprueba'";
	...
	$pdf->Cell(64,5,'',0,0,'C');
	$pdf->Cell(60,5,'JEFE DE DEPARTAMENTO',0,1,'C');	
	$pdf->Cell(190,5,$linea,0,1,'C');		
	...
	$nom_pdf="../fpdf/pdf/PlanNece_".$_GET['periodo']."_".$_GET['ano'].".pdf";
