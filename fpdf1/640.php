...
Cuando se realicen reintegros por concepto de gastos basicos de participantes, reste ese valor de los gastos básicos de participantes, RIN_CPFH8
*/
...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$consulta_unidad = "select sigla from cx_org_sub where subdependencia = '".$uni_usuario."'";
	...
	while($f<$nreg_cur_pag=odbc_fetch_array($cur1))	
	{
		...
		$lin = $lin + 5;
		$total = $total + $gastos;    //odbc_result($cur1,5);
		$f++;
	}
	$actual=$pdf->GetY();	
	...
	$consulta1 = "SELECT * from cx_rel_dis where conse1 = '".odbc_result($cur,1)."' and tipo = 'N' and consecu='".$consecu."' and ano= '".$_GET['ano']."'";
	$cur1 = odbc_exec($conexion,$consulta1);
