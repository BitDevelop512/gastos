...
else
{
	...
	$pdf->Cell(50,5,$comprobante,B,0,'C');
	$pdf->Cell(50,5,'  4. VALOR',0,0,'L');
	$pdf->Cell(52,5,'$'.$valor,1,1,'C');
	$pdf->Cell(39,5,'5. ORDOP.',0,0,'');
	$pdf->Cell(70,5,$nordop.' - '.$ordop,B,0,'L');
	$pdf->Cell(30,5,'  6. MISIÓN',0,0,'L');
	$pdf->Cell(52,5,$mision,B,1,'C');
	...
	$consulta1 = "select * from cx_rel_dis where conse1 = '".odbc_result($cur,1)."' and tipo = 'S' and consecu = '".$cosecutivo."' and ano= '$ano'";
	...
	$pdf->RoundedRect(9,$actual,192,5,0,'R');
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(146,5,'SUMAN GASTOS SIN FACTURA',0,0,'R');   //SOPORTES
	$pdf->Cell(45,5,'$'.number_format($total,2),1,1,'R');
	$pdf->SetFont('Arial','',8);
	$total_gastos = $total_gastos + $total;
	...
	else
	{
		...
		$c_comandante = $comandante1[1]; //substr($comandante1[1],0,-1);
	}  //if

	$actual = $pdf->GetY();
	$pdf->RoundedRect(9,$actual,96,25,0,'');
	$pdf->RoundedRect(105,$actual,96,25,0,'');
	$pdf->Ln(13);
	...
	$nom_pdf="../fpdf/pdf/Relaciones/".$_GET['ano']."/InfGas_".trim($sig_usuario)."_".$_GET['conse']."_".$_GET['ano'].".pdf";
