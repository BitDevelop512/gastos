...
else
{
	...
	$fechacomp = substr(odbc_result($cur1,2),0,10);
	...
	{
		$consulta_pi = "select * from cx_pla_inv where unidad = '".$_GET['unidad']."' and ano='".$_GET['ano']."' and periodo='".$_GET['periodo']."'";
	...
	$pdf->Cell(190,5,'RECURSOS AUTORIZADOS UNIDADES, DEPENDENCIAS O SECCIONES',0,0,'C');
	...
	$pdf->Ln(4);	
	$pdf->RoundedRect(9,$actual+40,15,5,0,'DF');
	$pdf->Cell(14,5,'ITEM',0,0,'C');
	$pdf->RoundedRect(24,$actual+40,130,5,0,'DF');
	$pdf->Cell(130,5,'CONCEPTOS DEL GASTO AUTORIZADOS',0,0,'C');
	$pdf->RoundedRect(154,$actual+40,47,5,0,'DF');
	$pdf->Cell(47,5,'V/R AUTORIZADO',0,1,'C');
	$pdf->RoundedRect(9,$actual+45,15,5,0,'');	
	$pdf->Cell(14,5,'1.2',0,0,'C');
	$pdf->RoundedRect(9,$actual+45,192,5,0,'');
	$pdf->Cell(154,5,'GASTOS EN ACTIVIDADES DE INTELIGENCIA Y CONTRAINTELIGENCIA',0,1,'');
	...
	while($i<$row=odbc_fetch_array($cur2))
	{
		...
		if (odbc_result($cur2,6) == 1)
		{
			...
			$pdf->Cell(130,5,"Misión: ".trim(odbc_result($cur2,4))." - ".trim(odbc_result($cur_cmp,3)),1,0,'L');
			$val = substr(str_replace(',','',trim(odbc_result($cur2,5))),0);
			$sval_gastos = $sval_gastos + $val;
			$pdf->Cell(47,5,wims_currency($val),1,1,'R');
		}
		else
		{
			if ($bol == 0)
			{
				...
				$pdf->Ln(5);
				$actual=$pdf->GetY();
				$pdf->RoundedRect(9,$actual,15,5,0,''); 
				$pdf->Cell(14,5,'1.3.',0,0,'C');
				$pdf->RoundedRect(9,$actual,192,5,0,'');  
				$pdf->Cell(154,5,'PAGO INFORMACIONES',0,1,'');					
				$bol = 1;
				$item = 0;
			}
			$item++;
			...
			$pdf->RoundedRect(9,$actual,192,5,0,''); 
			$pdf->Cell(130,5,odbc_result($cur2,4),1,0,'');
			$val = substr(str_replace(',','',trim(odbc_result($cur2,5))),0);
	...
	$pdf->RoundedRect(154,$actual,47,5,0,'');  
	$pdf->Cell(47,5,wims_currency($sval_pagos),1,1,'R');   
	$pdf->Ln(4);
	$actual=$pdf->GetY();
	$pdf->RoundedRect(9,$actual,15,5,0,'');	
	$pdf->Cell(14,5,'1.4',0,0,'C');
	$pdf->RoundedRect(9,$actual,145,5,0,'');  
	$pdf->Cell(143,5,'TOTAL RECURSOS AUTORIZADOS UNIDAD/ÁREA INT/Cl',0,0,'');	
	$pdf->RoundedRect(154,$actual,47,5,0,'');  
	$vlr2 = trim($sval_gastos+$sval_pagos);
	$pdf->Cell(47,5,wims_currency($vlr2),0,1,'C');   		

	$pdf->Ln(4);
	$pdf->RoundedRect(9,$actual,145,5,0,'');  
	$pdf->SetFont('Arial','B',8);	
	$pdf->Cell(144,5,'TOTAL RECURSOS GASTOS RESERVADOS AUTORIZADOS',0,0,'R');
	$pdf->Cell(47,5,wims_currency($sval_gastos+$sval_pagos),1,1,'R');
	$pdf->SetFont('Arial','',8);

	$actual=$pdf->GetY()+5;
	if ($actual>=259.00125) $pdf->addpage();
	$pdf->Ln(5);	
	$pdf->RoundedRect(9,$pdf->GetY(),192,5,0,'DF');
	$pdf->Cell(190,5,'INSTRUCCIONES GENERALES',0,0,'');
	$pdf->Ln(5);
	...
	$texto1 = "2. Los soportes que justifican pagos y gastos, deberán ser rendidos en forma consolidada a la Unidad Ejecutora con plazo: ".$f_plazo.", cumpliendo los requisitos y términos determinados en la normatividad de gastos reservados (Directiva Permanente No. ".trim($directiva)." de 2019)";
    $texto2 = "3. INSTRUCCIONES ADICIONALES: ".$otro;

	$pdf->Multicell(180,5,$texto,0,'J');
	$pdf->Multicell(180,5,$texto1,0,'J');
	$pdf->Multicell(180,5,$texto2,0,'J');
	...
	//Busca imagen de la firma $n_firma2
	//$n_firma2 = "JAIME ALBERTO MORALES (CX)";
	$consulta_fr = "select firma, usuario from cx_usu_web where nombre = '".$n_firma2."'";
	...
	$pdf->Ln(4);
	//$pdf->Image($file,46,$actual-20,30,26);
		
	//Busca imagen de la firma $n_firma3
	//$n_firma3 = "JAIME ALBERTO MORALES (CX)";
	$consulta_fr = "select firma, usuario from cx_usu_web where nombre = '".$n_firma3."'";
	$cur_fr = odbc_exec($conexion,$consulta_fr);
	if (odbc_num_rows($cur_fr) > 0)
	{
	...
	//$pdf->Image($file,141,$actual-20,30,26);
	...
	$pdf->Cell(95,4,$n_firma2,0,0,'C');
	$pdf->Cell(95,4,$n_firma3,0,1,'C');
	$pdf->Cell(95,4,$c_firma2,0,0,'C');
	$pdf->Cell(95,4,$c_firma3,0,1,'C');
	...
	$pdf->Cell(14,4,'Elaboró:',0,0,'');
	$pdf->Cell(87,4,$n_firma4,0,0,'L');
	$pdf->Cell(14,4,'Revisó:',0,0,'');
	$pdf->Cell(75,4,$n_firma1,0,1,'L');
	$pdf->Cell(190,4,$linea,0,1,'C');
		
	$nom_pdf = "pdf/InfAuto_".$num_informe."_".trim($sigla)."_".$_GET['periodo']."_".$_GET['ano'].".pdf";
	...
	$nom_pdf="../fpdf/pdf/InfAut_".$_GET['unidad']."_".$_GET['informe']."_".$_GET['periodo']."_".$_GET['ano'].".pdf";
	$pdf->Output($nom_pdf,"F");
