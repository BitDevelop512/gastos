...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$this->Cell(55,5,'PLAN DE INVERSION',0,0,'C');
		...
		function Footer()
		{
  			...
  			$this->Cell(190,4,'SIGAR 1 - '.$fecha1,0,1,'');
  			$this->Ln(-4);
  			$this->SetFont('Arial','B',8);
  			$this->SetTextColor(255,150,150);
  			$this->Cell(190,5,'SECRETO',0,1,'C');
	...
	$consulta_pla = "select * from cx_pla_inv where unidad='$uni_usuario' and periodo='$periodo' and ano='$ano' and tipo='1' and estado NOT IN ('','X','Y') order by conse";
  	$cur_pla = odbc_exec($conexion,$consulta_pla);
	$ban = 0;
	if (odbc_num_rows($cur_pla) == 0) $ban = 1;
	if ($adm_usuario == "6")
	{
		$query = "SELECT dependencia FROM cx_org_sub WHERE subdependencia='".$_SESSION["uni_usuario"]."'";
		...
		$consulta = "SELECT * FROM cx_pla_inv WHERE unidad IN ($numero) AND periodo='".$_GET['periodo']."' AND ano='".$_GET['ano']."' AND tipo='1' AND estado NOT IN ('','X','Y') ORDER BY conse";
	}
	else
	{
		$consulta = "SELECT * FROM cx_pla_inv WHERE unidad='".$_SESSION["uni_usuario"]."' AND periodo='".$_GET['periodo']."' AND ano='".$_GET['ano']."' AND tipo='1' AND estado NOT IN ('','X','Y') ORDER BY conse";
	}   //if
	...
	$consulta17 = "select nombre, cargo from cx_usu_web where unidad='".$_SESSION["uni_usuario"]."' and admin = 4";  //no aplica para unidades de combate
	$cur17 = odbc_exec($conexion,$consulta17);
	$firma_autoriza = trim(odbc_result($cur17,1));
	$cargo_autoriza = trim(odbc_result($cur17,2));
	...
	while($u<$row=odbc_fetch_array($cur))
	{
		...
		$aprueba = odbc_result($cur,25);
		if ($aprueba == "0") $n_aprueba = "";
		else
		{
			$consulta14 = "select nombre from cx_usu_web where conse='$aprueba'";
			$cur14 = odbc_exec($conexion,$consulta14);
			$n_aprueba = trim(odbc_result($cur14,1));
		}   //if

		$revisa = odbc_result($cur,26);
		if ($revisa == "0") $n_revisa = "";
		else
		{
			$consulta15 = "select nombre from cx_usu_web where conse='$revisa'";
			$cur15 = odbc_exec($conexion,$consulta15);
			$n_revisa = strtr(trim(odbc_result($cur15,1)), $sustituye);
		}   //if
...
*/
		...
		$u++;  
		$elabora = explode("»",trim(odbc_result($cur,45)));
		$n_elabora = substr($elabora[0],0,-1);
		$c_elabora = substr($elabora[1],0,-1);
	}   //while
	...
	$pdf->Cell(190,5,$firma_autoriza,0,1,'C');     //$n_aprueba
	...
	$nom_pdf="../fpdf/pdf/PlanInvGRCon_".trim($sig_usuario)."_".$_GET['periodo']."_".$_GET['ano'].".pdf";
