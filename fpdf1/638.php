...
else
{
	...
	class PDF extends PDF_Rotate
	{
		...
		function Footer()
		{
			...
			$this->Cell(190,4,'SIGAR0 - '.$fecha1,0,1,'');
			$this->Ln(-4);
			$this->SetFont('Arial','B',8);
			$this->SetTextColor(255,150,150);
			$this->Cell(190,5,'SECRETO',0,1,'C');
	...
	//$consulta_os = "select * from cx_org_sub where subdependencia='$unidad_p'";
	...
	elseif ($num_unidad >= 1 and $num_unidad <= 3) 
	{
		$n_reviso = "";
		$c_reviso = "";
	}
	...
	elseif ($num_unidad >= 4 and $num_unidad <= 21) 
	{
		...
		{
			if ($row['firma4'] != "") $fir_ela = $row['firma4'];
			else $fir_ela = $row['firma5'];
		}  //if
		$aprobo = explode("»",$fir_ela);
		$n_aprobo = substr($aprobo[0],0,-1);
		$c_aprobo = substr($aprobo[1],0,-1);
	...
	$n_datos = "Factor de Amenaza: ".$n_factor." - Estructura: ".$n_estructura." - OMAVE - OMINA - OMIRE: ".$n_oms;
	...
	while($i<$row=odbc_fetch_array($cur3))
	{
		...
		$consulta4 = "select * from cx_pla_gad where conse1='$conse' and interno='$i' and ano='$ano'";
