...
else
{
	...
	if ($t_comite == 1)   //central
	{
		...
		$s_unidad = trim(odbc_result($cur_os,4));
		...
		if (trim(odbc_result($cur_reg,17)) == "") $vlr_solicitado = "0.00";
		else $vlr_solicitado = substr(str_replace(',','',trim(odbc_result($cur_reg,17))),0);
		...
		$asunto = "TRATA DEL ESTUDIO QUE HACE EL COMITÉ CENTRAL DE RECOMPENSAS AL EXPEDIENTE REMITIDO POR EL COMANDO DEL CAIMI ";  //.$sigla_uni.", ";
	...
	else    //Regional
	{
		...
		if ($tipo == 1)
		{
			//$consulta_rev = "select * from cx_reg_rev where consecu = '".$registro."' and unidad = '".$unidad."' and ano = '".$ano1."' order by conse";
		...
		$pdf->RoundedRect(9,$actual,192,10,0,'DF');
		$pdf->MultiCell(191,5,'1. CERTIFICACIÓN DE LA EXISTENCIA Y REGISTRO DE LA FUENTE HUMANA Y DE LA INFORMACIÓN SUMINISTRADA EN BASES DE DATOS DE INTELIGENCIA:',0,'L');
		$certi = "Según la(s) certificación(es) anexa(s) a folio(s) ".$constancia." existe(n) una(s) fuente(s) que aportó(aron) información(es) que condujo al planeamiento y/o desarrollo de la orden de operaciones ".$ordop.", orden fragmantaria ".$fragmenta.", adelantada por tropas del ".$s_unidad_afec." - ".$n_unidad_afec.".";
	...
	ob_end_clean();
	if ($t_comite == 1) $nom_pdf = "../fpdf/pdf/Actas/ActaComCen_".$s_unidad."_".$conse."_".$ano_uni_rec.".pdf";
	else $nom_pdf = "../fpdf/pdf/Actas/ActaComReg_".$s_unidad."_".$conse."_".$ano_uni_rec.".pdf";
echo $nom_pdf." fpdf1<br>";
