...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$this->RotatedText(55,200,$sigla,35);   //$this->RotatedText(55,200,$sig_usuario,35);
	...
	$consulta = "select * from cx_org_sub where subdependencia = '".$_SESSION["uni_usuario"]."'";
	$cur = odbc_exec($conexion,$consulta);
	$sigla1 = trim(odbc_result($cur,4));
	$nom_unidad = trim(odbc_result($cur,6));
	$ciudad_uni = trim(odbc_result($cur,5));

	if ($_GET['tipo'] == '1')
	{
		$pag_inf = "  X";
		//$consulta1 = "select * from cx_act_inf where conse = '".$_GET['conse']."' and unidad = '".$_SESSION["uni_usuario"]."' and ano = '".$_GET['ano']."'";
		$consulta1 = "select * from cx_act_inf where conse = '".$_GET['conse']."' and ano = '".$_GET['ano']."'";
		...
		if (strlen($observacion1) == 0) 
		{
			...
			else $texto_obs = "Con la realización de este pago, la unidad quedó a Paz y Salvo con la fuente.\nLa fuente no firma el acta por su ubicación, no obstante, en constancia se anexa certificado de giro realizado en el banco XXXX del valor descrito en acta.\n\n";
		}
	...
	{
		$pag_rec = "  X";
		$consulta1 = "select * from cx_act_rec where conse = '".$_GET['conse']."' and ano = '".$_GET['ano']."'";
		$cur1 = odbc_exec($conexion,$consulta1);
		$fecha = substr(odbc_result($cur1,2),0,10);
	...
	$pdf->Cell(60,4,'Elaboró:    '.utf8_decode($elabora),0,1,'');
	$pdf->Cell(190,3,$linea,0,1,'C');
	...
	$nom_pdf="../fpdf/pdf/Actas/ActaPagFue_".$sig_usuario."_".$_GET['conse']."_".$_GET['ano'].".pdf";
