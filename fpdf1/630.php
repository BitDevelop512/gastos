...
//echo $consulta_rman."<br>";
	...
	while($i<$row=odbc_fetch_array($cur1))
	{
		...
		$pdf->Cell(34,5,$sigla,1,0,'C');
		if ($concepto_gir == '10')
		{ 
			$pdf->Cell(67,5,'$0.00',1,0,'R');
			$pdf->Cell(67,5,'$'.number_format($pagos_inf,2),1,1,'R');
			$subt_gastos = $subt_gastos + 0;
			$subt_pagos = $subt_pagos + $pagos_inf;
		}
		else
		{
			$pdf->Cell(67,5,'$'.number_format($gastos_act,2),1,0,'R');
			$pdf->Cell(67,5,'$'.number_format($pagos_inf,2),1,1,'R');
	...
	$consulta_rr = "select * from cx_reg_rec where conse = '".$solicitud."' and unidad='".$uni1."'";
//echo $consulta_rr."<br>"; 
...
//echo $concepto_rec."<br>"; 
	...
	//$firma1 = "JAIME ALBERTO MORALES (CX)";
	$consulta_fr = "select firma, usuario from cx_usu_web where nombre = '".$firma1."'";
	...
	$consulta2="select usuario, nombre, cargo, ciudad from cx_usu_web where usuario='STE_DIADI'";
	$cur2 = odbc_exec($conexion,$consulta2);
	$elaboro = trim(odbc_result($cur2,2));
	$pdf->Cell(15,4,'Elaboró:',0,0,'');
	$pdf->Cell(30,3,strtr($elaboro, $sustituye),0,1,'L');
	$pdf->Cell(190,4,$linea,0,1,'C');
	...
	$nom_pdf = "../fpdf/pdf/Informes/".$_GET['ano']."/InfGiro_".$_GET['informe']."_".$_GET['periodo']."_".$_GET['ano'].".pdf";
