...
else
{
	...
	$linea = str_repeat("_",285);

	function control_salto_pag($actual1)
	{
		global $pdf;
		$actual1=$actual1+5;
		if ($actual1>=172.00125) $pdf->addpage();    //196.00125
	} //control_salto_pag
	...
	$pdf->Cell(142,3,'INFORMACIÓN DE EJECUICIÓN MENSUAL'.$recu,0,0,'');
	for ($j = 1; $j <= 8; $j++)
	{
	...
	while($u<odbc_fetch_array($cur))
	{
		...
		$pdf->Cell(17,3,wims_currency(odbc_result($cur,17)),1,1,'R'); //saldo 		(15)		
	...
	$pdf->Ln(2);
	//$pdf->Image($file,130,$actual-20,30,26);
	...
	$pdf->Ln(2);
	//$pdf->Image($file,224,$actual-20,30,26);
