...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$this->SetFont('Arial','',8);
			$this->Cell(36,5,'FO-JEMPP-CEDE2-638-2',0,1,'');
		...
		function Footer()
		{
  			...
  			$this->Cell(190,4,'SIGAR 2 - '.$fecha1,0,1,'');
  			$this->Ln(-4);
  			$this->SetFont('Arial','B',8);
  			$this->SetTextColor(255,150,150);
  			$this->Cell(190,5,'SECRETO',0,1,'C');
	...
	$consulta1 = "select sigla, unic, ciudad, tipo, unidad from cx_org_sub where subdependencia='".$subdependencia."'";
	$cur1 = odbc_exec($conexion,$consulta1);
	...
	$consulta = "select * from cx_pla_inv where conse in ($cad_planes) and ano='".$_GET['ano']."' and estado <> 'F' order by conse";
...
//echo $subdependencia."<br>";
	...
	elseif ($subdependencia >= 4)    //firmas de cx_pla_con
	{
...
//echo $consulta_pc."<br>";
			...
			$row_pc = odbc_fetch_array($cur_pc);
			$frm3 = $row_pc['firma3'];
			$frm4 = $row_pc['firma4'];
			$frm5 = $row_pc['firma5'];
