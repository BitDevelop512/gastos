...
{
	if ($nummero >= 1000 && $nummero < 2000) $numm = "MIL ".(centena($nummero%1000));
...
function cienmillon($numcmeros)
{
	...
	if ($numcmeros == 0) $num_letracms = "MILLONES DE ";
	return $num_letracms;
}//cienmillon	

function milmillon($nummierod)
{
	if ($nummierod >= 1000000000 && $nummierod < 2000000000) $num_letrammd = "MIL ".(cienmillon($nummierod%1000000000));
...
function convertir($numero)
{
	...
	if ($numf1 != "")
	{
		...
		if ($tempnum[0] != "")
		{
			...
			$bcadena = array("MILLONES", "MILLON");
			$encuentra = False;
			foreach ($bcadena as $valor)
			{
				$pos = strpos($numf, $valor);
				if ($pos != false) $encuentra = True;
			}   //for

			if ($encuentra == True)
			{
				$lc = substr($numf,-4);
				if ($lc == 'LON ' or $lc == 'NES ') $numf = $numf." DE ";
			}   //if
				
			$textend = $numf;
			$textend .= $ps.' MCTE CON ';
			if ($tempnum[1] == "" || $tempnum[1] >=  100) $tempnum[1] = "00" ;
			$textend .= $tempnum[1] ;
			$textend .= "/100";
		}   //if
	}   //if
