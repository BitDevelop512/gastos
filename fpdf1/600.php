<?php
/* 600.php
   ACTA EVALUACIÓN COMITÉ RECURSOS ADICIONALES.
...
else
{
	...
	function control_salto_pag($actual1)
	{
		global $pdf;
		$actual1=$actual1+5;
		if ($actual1>=258.00125) $pdf->addpage();
	} //control_salto_pag
	...
	$consulta_usuweb = "select usuario, nombre, cargo from cx_usu_web where nombre like '%".substr($n_elaboro,5)."%' order by conse DESC";    //usuario = '".$usu_usuario."'";
	$cur_usuweb = odbc_exec($conexion,$consulta_usuweb);
	$c_elaboro = trim(odbc_result($cur_usuweb,3));
	
	//Las firmas se usarán de acuerdo a la solicitud de Consuelo Martinez y no a las registradas en el registro.
	$num_fir = explode("|",$fir);
	...
	$actual=$pdf->GetY();
    if (strlen($asunto) <= 110) $aa = 5;
    else $aa = ceil(strlen($asunto)/100)*5;
	
	$pdf->RoundedRect(9,$actual,192,$aa,0,'');
	$pdf->Cell(39,5,'A S U N T O:',0,0,'');
	$pdf->Multicell(152,5,$asunto,1,'L');
    ...
    while($rg<$row=odbc_fetch_array($cur1))
    {
		...
		$x = $pdf->Getx();
		$y = $pdf->Gety();
		$concepto = "\n".$concepto." ".$ordopmision;
		$texto_int = "\n".$texto_int;
		$l_alto = max(strlen($concepto), strlen($texto_int));
		$alto1 = ceil($l_alto / 30);
		if ($alto1 == 1 or $alto1 == 2) $alto = 15;
		else $alto = ceil($alto1 * 4.6);

		$pdf->RoundedRect(9,$actual,12,$alto,0,'D');
		$pdf->MultiCell(12,$alto,$radicado,0,'C');
		$pdf->RoundedRect(21,$actual,15,$alto,0,'D');
		$pdf->SetXY($x+11,$y);	
		$pdf->Multicell(15,$alto,$unidadsol,0,'C');
		$pdf->RoundedRect(36,$actual,21,$alto,0,'D');
		$pdf->SetXY($x+26,$y);	
		$pdf->Multicell(22,$alto,$unidadcen,0,'C');
		$pdf->RoundedRect(57,$actual,48,$alto,0,'D');
		$pdf->SetXY($x+47,$y);
		$pdf->Multicell(48,3,$concepto,0,'C');
		$pdf->RoundedRect(105,$actual,48,$alto,0,'D');		
		$pdf->SetXY($x+95,$y);
		$pdf->Multicell(48,3,$texto_int,0,'C');
		$pdf->RoundedRect(153,$actual,24,$alto,0,'D');				
		$pdf->SetXY($x+143,$y);
		$pdf->Multicell(24,$alto,'$'.number_format($vsol,2),0,'R');
		$pdf->RoundedRect(177,$actual,24,$alto,0,'D');				
		$pdf->SetXY($x+167,$y);
		$pdf->Multicell(24,$alto,'$'.number_format($vaprobado,2),0,'R');
		$tvsol = $tvsol + $vsol;
		$vsol = 0;
	...
	$pdf->Ln(2);
	$pdf->Multicell(191,5,$sustentacion,0,'J');    
	$pdf->Ln(1);
	...
	$pdf->Ln(2);
	$pdf->Multicell(191,5,$anexos.$anexos1,0,'L');
	$pdf->Cell(190,3,$linea,0,1,'C');
	...
	$pdf->Ln(1);
	$pdf->Multicell(191,5,$observacion,0,'L');
	$pdf->Cell(190,3,$linea,0,1,'C');
