...
if (is_ajax())
{
  ...
  // Se consulta todos los valores autorizados por unidad centralizadora
  if ($comando == "0")
  {
  ...
  {
    $query = "SELECT * FROM cx_val_aut WHERE unidad!='$uni_usuario' AND unidad IN ($unidades) AND periodo='$periodo' AND ano='$ano' AND estado='V' ORDER BY uom, depen, unidad";
  }
  ...
  {
    $salida->rows[1]['unidad'] = odbc_result($cur,4);
    $salida->rows[1]['periodo'] = odbc_result($cur,5);
    $salida->rows[1]['ano'] = odbc_result($cur,6);
    $salida->rows[1]['sigla'] = odbc_result($cur,7);
    $salida->rows[1]['gastos'] = odbc_result($cur,8);
    $salida->rows[1]['pagos'] = odbc_result($cur,9);
    $salida->rows[1]['total'] = odbc_result($cur,10);
    $salida->rows[1]['conso'] = "0";
    $salida->rows[1]['conso1'] = "0";
    $i = 2;
  }
...
// 03/04/2025 - Ajuste order unidades por cambio de divisiones
?>