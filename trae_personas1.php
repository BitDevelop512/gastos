...
if (is_ajax())
{
	...
	if (($paso == "1") or ($paso == "2")  or ($paso == "3")  or (($paso == "9") and (($uni_usuario == "6") OR ($uni_usuario == "7"))))
	{
		...
		$v3 = "DIADI";
		$v4 = "CEDE2";
		$query00 = "SELECT n_dependencia FROM cv_unidades WHERE subdependencia='$uni_usuario'";
		$sql00 = odbc_exec($conexion, $query00);
		$v5 = trim(odbc_result($sql00,1));
		if (($usu_usuario == "JEF_CEDE2") or ($log_usuario == "JEF_CEDE2"))
		{
		...
		else
		{
			...
			else
			{
				if ($adm_usuario == "21")
				{
					$query = "SELECT * FROM cx_usu_web WHERE usuario!='$usu_usuario' AND usuario LIKE '%$v5%'";
				}
				else
				{
					$query = "SELECT * FROM cx_usu_web WHERE usuario!='$usu_usuario' AND ((usuario LIKE '%$v2%') OR (usuario LIKE '%$v3%') OR (usuario LIKE '%$v4%') OR (usuario LIKE '%$v5%'))";
				}
			}
		}
	}
