...
if (is_ajax())
{
	...
	if ($tipo == "1")
	{
		...
		{
			$pregunta = "SELECT unidad, compania FROM cx_pla_tra WHERE placa='$b[$i]'";
			$sql2 = odbc_exec($conexion, $pregunta);
			$unidad1 = odbc_result($sql2,1);
			$compania1 = trim(odbc_result($sql2,2));
			...
			$graba = "INSERT INTO cx_tra_mom (conse, usuario, unidad, unidad1, ano, tipo, codigo, placa, acto, fechat, observaciones, elaboro, alea, compania, compania1) VALUES ('$consecu', '$usu_usuario', '$unidad1', '$unidad', '$ano', '$tipo', '$a[$i]', '$b[$i]', '$acto', '$fecha', '$observaciones', '$elaboro', '$alea', '$compania', '$compania1')";
...
// 12/03/2024 - Ajuste grabacion traslados entre compañias
?>