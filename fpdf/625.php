...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$consulta_ci = "select * from cx_com_ing where ingreso = '".$ingreso."' and ano = '".$ano."' and unidad = '".$unidad."'";
			$cur_ci = odbc_exec($conexion,$consulta_ci);
			$sigla_ci = substr(trim(odbc_result($cur_ci,3)),4);
			$unidad_ci = trim(odbc_result($cur_ci,4));
			$estado = trim(odbc_result($cur_ci,5));
			$fecha_ci = str_replace("/", "-", substr(trim(odbc_result($cur_ci,2)),0,10));			
			
			$consulta1 = "select sigla, nombre, sigla1, nombre1, fecha from cx_org_sub where subdependencia= '".$unidad_ci."'";
			...
			if ($sigla1 <> "") if ($fecha_ci >= $fecha_os) $sigla = $sigla1;
	...
	$consulta = "select * from cx_com_ing where ingreso = '".$ingreso."' and ano = '".$ano."' and unidad = '".$unidad."'";
	...
	if ($unidad_centralizadora == 1)
	{
		...
		if ($id_cuenta == 3)
		{
			...
			if ($concepto == 19)  //PRESUPUESTO MENSUAL CEDE2
			{
				$cta_DTN = '1110050045';  
				$banco1 = 'GASTOS GENERALES';	
				$cta_func = '4705080020';
	...
	$pdf->Cell(190,4,$n_vobo,0,1,'C');
	$pdf->Cell(190,4,$c_vobo,0,1,'C');
	$pdf->SetFont('Arial','',7);
	...
	$consulta1="select * from Cx_org_sub where subdependencia = '".$unidad_centralizadora."'";
	$cur1 = odbc_exec($conexion,$consulta1);
	$nom_pdf = "../fpdf/pdf/Comprobantes/".$_GET['ano']."/CompIng_".trim(odbc_result($cur1,4))."_".$_GET['ingreso']."_".$_GET['ano'].".pdf";
