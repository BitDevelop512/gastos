...
/* 634.php (lib_aux_bco.php)
   ...
   (pág 209 - "Dirtectiva Permanente No. 00095 de 2017.PDF")
	...
	01/07/2023 - Solo muestra las cuentgas para el CEDE2, las demás unidades solo tienen una cuenta de Gastos y no la muestra.
	- SE HACE CONTROL DEL CAMBIO DE LA SIGLA DE LA UNIDAD. Jorge Clavijo
	- 22/05/2024 - Se deshabilita permanentemente la grabación del formato en PDF. Jorge Clavijo
*/
...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$consulta_lb = "select * from cv_lib_ban where periodo = '".$_GET['periodo']."' and ano = '".$_GET['ano']."' and unidad = '".$uni_usuario."'  and cuenta = '".$_GET['cuenta']."' order by fecha, tipo1 DESC"; 
			$cur_lb = odbc_exec($conexion,$consulta_lb);
			$unidad = trim(odbc_result($cur_lb,3));
			$fecha_lb = str_replace("/", "-", substr(trim(odbc_result($cur_lb,2)),0,10));			
	...
	$pdf->Cell(110,5,$sig_usuario,B,0,''); 
	...
	$pdf->Cell(40,5,' de '.$n_meses[$_GET['periodo'] - 1].' de '.$_GET['ano'],0,0,'C');
	...
	if ($_GET['cuenta'] == '1')
	{
		if ($uni_usuario == '1')
		{
			...
			$pdf->Cell(40,5,$nom_cta.' - '.$cta,B,1,'L');
		}   //if			
	...
	$pdf->Cell(24,4,'NÚMERO',0,0,'C');
	$pdf->Cell(20,4,'',L,0,'C');
	$pdf->Cell(29,4,'NÚMERO',L,0,'C');
	$pdf->Cell(70,4,'',L,0,'C');
	$pdf->Cell(45,4,'',L,0,'C');
	$pdf->Cell(45,4,'',L,0,'C');
	$pdf->Cell(4,4,'',L,1,'C');
	...
	$consulta_libban = "select * from cv_lib_ban where periodo = '".$_GET['periodo']."' and ano = '".$_GET['ano']."' and unidad = '".$uni_usuario."'  and cuenta = '".$_GET['cuenta']."' order by fecha, tipo1 DESC"; 
	...
	{
		if (odbc_result($cur_libban,10) == '2')
		{
			...
			$unidad_centralizadora = odbc_result($cur_libban,3);
			$concepto = odbc_result($cur_soporte,7);
		...
		$data[] = array($compf, $fecha, $n_cheque, $conceptof, $credito, $debito, $fecha_all);
		$i++;
	...
	{	
		...
		if ($_GET['cuenta'] == $cta_final_cm)
		{
			...
			$data[] = array($egreso_cm, $fecha_cm, $n_cheque_cm, $conceptof, $credito_cm, $debito_cm, $fecha_all);
		}   //if
		$i++;
		control_salto_pag($pdf->GetY());
	} //while
	...
	for ($ax=0;$ax<=count($data)-1;$ax++) $aux[$ax] = $data[$ax][6];
	...
	for ($ax=0;$ax<=count($data)-1;$ax++)
	{
		...
		$pdf->Cell(45,4,wims_currency($data[$ax][4]),L,0,'R');
		$pdf->Cell(45,4,wims_currency($data[$ax][5]),L,0,'R');
		$saldo =  $saldo + $data[$ax][4] - $data[$ax][5];
		$pdf->Cell(45,4,wims_currency($saldo),L,1,'R');
		control_salto_pag($pdf->GetY());		
	}   //for
	...
	$pdf->Cell(139,3,$n_jem,0,1,'C');
	$pdf->Cell(139,3,$c_reviso,0,0,'C');
	$pdf->Cell(139,3,$c_jem,0,1,'C');
	$pdf->Cell(278,8,'',0,1,'L');	

	if ($pdf->GetY() >= 171.00125)
	{
		$pdf->addpage();
		$pdf->Ln(24);
	}   //if
...
/*
	$nom_pdf="pdf/Libros/".$_GET['ano']."/AuxBan_".trim($sig_usuario)."_".$_GET['periodo']."_".$_GET['ano']."_".$_GET['cuenta'].".pdf";
