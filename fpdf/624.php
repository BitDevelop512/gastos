...
/* 624.php
   ...
   (pág 158 - "Dirtectiva Permanente No. 00095 de 2017.PDF")
	...
	25/02/2025 - Se adiciona el nombre del recurso a la autorización- - Requerimiento Jorge Clavijo 
*/
...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$consulta_ce = "select * from cx_com_egr where egreso = '".$egreso."' and ano = '".$ano."' and unidad = '".$unidad."'";   
			$cur_ce = odbc_exec($conexion,$consulta_ce);
			$sigla_ce = substr(trim(odbc_result($cur_ce,3)),4);
			$unidad_ce = trim(odbc_result($cur_ce,4));
			$estado_ce = trim(odbc_result($cur_ce,8));
			$fecha_ce = str_replace("/", "-", substr(trim(odbc_result($cur_ce,2)),0,10));			
			
			$consulta1 = "select sigla, nombre, sigla1, nombre1, fecha from cx_org_sub where subdependencia= '".$unidad_ce."'";
			...
			if ($sigla1 <> "") if ($fecha_ce >= $fecha_os) $sigla = $sigla1;
	...
	$consulta = "select * from cx_com_egr where egreso='$egreso' and ano='$ano' and unidad = '".$unidad."'";
	$cur = odbc_exec($conexion,$consulta);
	$fechacomp = substr(odbc_result($cur,2),0,10);
	$fecha_ce = str_replace("/", "-", $fechacomp);	
	...
	$autonum = trim(odbc_result($cur,25));	
	...
	$nom_concepto = odbc_result($cur_gas,2);

	//25/02/2025 - Se adiciona el nombre del recurso a la autorización- - Requerimiento Jorge Clavijo
	if ($autonum == 6)
	{
		$consulta_va = "select * from cx_val_aut1 where inf_giro = '".$giro."'";
		$cur_va = odbc_exec($conexion,$consulta_va);
		$ano_va = trim(odbc_result($cur_va,6));
		$solicitud_va = trim(odbc_result($cur_va,21));

		$consulta_pi = "select * from cx_pla_inv where conse = '".$solicitud_va."' and ano = '".$ano_va."'";
		$cur_pi = odbc_exec($conexion,$consulta_pi);
		$recurso = trim(odbc_result($cur_pi,58));
		
		$consulta_cr = "select * from cx_ctr_rec where codigo = '".$recurso."' and estado <> 'X'";
		$cur_cr = odbc_exec($conexion,$consulta_cr);
		if (odbc_num_rows($cur_cr) == 0) $n_recu = "";
		else $n_recu = " - ".odbc_result($cur_cr,2);
	}   //if
	...
	if ($concepto == '8' and $tpgasto == '2') $nom_concepto = "  PAGO DE INFORMACIÓN";   	//Se adiciona la linea, no estaba definida 09/05/2023.
	...
	//if ($concepto == '18' and $tpgasto == '1') $nom_concepto = "  POR DEFINIR";   	//Se adiciona la linea, no estaba definida 11/05/2023. Jorge Clavijo
	//if ($concepto == '18' and $tpgasto == '2') $nom_concepto = "  POR DEFINIR";   	//Se adiciona la linea, no estaba definida 11/05/2023. Jorge Clavijo
	...
	$pago = trim(odbc_result($cur_med,2)).":";	
	...
	$pdf->Cell(163,5,$autoriza1.$n_recu,B,1,'');
	...
	$pdf->Multicell(181,5,$num_sopo,B,'L');

	$linn = 116;
	if (strlen($num_autoriza) >= 131 and strlen($num_sopo) >= 131) $linn = 126;
	if (strlen($num_autoriza) <= 131 and strlen($num_sopo) >= 131) $linn = 121;
	if (strlen($num_autoriza) >= 131 and strlen($num_sopo) <= 131) $linn = 121;
	...
	$linn = $linn + 5;		
	$pdf->RoundedRect(9,$linn,105,5,0,'');
	if (strlen($num_autoriza) <= 131) $pdf->Ln(5);
	else $pdf->Ln(5);
	$pdf->Cell(15,5,'Uni/Dep/Sec',0,0,'C');
	$pdf->Cell(19,5,'Valor',L,0,'C');
	$pdf->Cell(16,5,'Uni/Dep/Sec',L,0,'C');
	$pdf->Cell(19,5,'Valor',L,0,'C');
	$pdf->Cell(16,5,'Uni/Dep/Sec',L,0,'C');
	$pdf->Cell(19,5,'Valor',L,1,'C');

	//Control cambio de sigla
	function verifica_sigla($v_sigla, $fecha_ce1)
	{
		require('../conf.php');
		if ($v_sigla != "")
		{
			...
			$cur1 = odbc_exec($conexion,$consulta1);
			$sigla_os = trim(odbc_result($cur1,1));
			$sigla1_os = trim(odbc_result($cur1,3));
			$fecha_os = str_replace("/", "-", substr(trim(odbc_result($cur1,5)),0,10));
			if ($sigla1_os <> "") if ($fecha_ce1 >= $fecha_os) $sigla_os = $sigla1_os;
	...
	$pdf->Cell(15,5,$d_datos1[0],0,0,'');
	$pdf->Cell(19,5,"$".$d_datos1[1],1,0,'R'); 
	$d_datos1 = explode("|",$d_datos[1]);
	$pdf->Cell(16,5,$d_datos1[0],1,0,'');
	$pdf->Cell(19,5,"$".$d_datos1[1],1,0,'R');
	$d_datos1 = explode("|",$d_datos[2]);		
	$pdf->Cell(16,5,$d_datos1[0],1,0,'');
	$pdf->Cell(19,5,"$".$d_datos1[1],1,1,'R');
	...
	$pdf->SetFont('Arial','',8);		
	$pdf->Cell(37,5,'   RETE FUENTE',0,0,'L');
	...
	$pdf->SetFont('Arial','',8);			
	$pdf->Cell(37,5,'   TOTAL RETENCIONES',0,0,'L');
	$pdf->Cell(12,5,'',0,0,'C');
	...
	$pdf->SetFont('Arial','',8);			
	$pdf->Cell(35,5,'VALOR NETO',0,0,'R');
	$pdf->Cell(14,5,'',0,0,'C');
...
*/
	...
	$frm1 = $n_ejecuto."\n".$c_ejecuto;
	$frm2 = $n_recibio."\n".$c_recibio;
	$pdf->RoundedRect(9,$actual+2,96,42,0,'');
	$pdf->RoundedRect(105,$actual+2,96,42,0,'');
	$pdf->Cell(95,17,'',0,0,'L');
	$pdf->Cell(93,17,'',0,1,'L');
	$x = $pdf->Getx();
	$y = $pdf->Gety();
	$pdf->Multicell(92,4,$frm1,T,'C');
	$pdf->SetXY($x+97,$y);
	$pdf->Multicell(92,4,$frm2,T,'C');
/*	
...
*/
	...
	$consulta1="select * from Cx_org_sub where subdependencia = '".$unidad_centralizadora."'";
	$cur1 = odbc_exec($conexion,$consulta1);
	$nom_pdf = "../fpdf/pdf/Comprobantes/".$_GET['ano']."/CompEgr_".trim(odbc_result($cur1,4))."_".$_GET['egreso']."_".$_GET['ano'].".pdf";
