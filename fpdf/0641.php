...
//echo $consulta."<br>";
		...
		//----------- firma1 ------------
		$firma1 = trim(odbc_result($cur,45));   //firma1
		$elaboro = explode("»",$firma1);
		$elaboro[0] = preg_replace('/[^A-Za-z0-9 *.ÑÁÉÍÓÚáéíóú]/', '', $elaboro[0]);
		$elaboro[1] = strtr($elaboro[1], $sustituye);
		$elaboro[1] = preg_replace('/[^A-Za-z0-9 *.ÁÉÍÓÚáéíóú]/', '', $elaboro[1]);
		if (substr(utf8_decode($cdo[0]), -1) == "?") $n_cdo = utf8_encode(substr($cdo[0],0,-1));
		if (substr(utf8_decode($elaboro[0]), -1) == "?") $n_elaboro = substr($elaboro[0],0,-1);
		else $n_elaboro = $elaboro[0];
		if (substr(utf8_decode($elaboro[1]), -1) == "?") $c_elaboro = substr($elaboro[1],0,-1);
		else $c_elaboro = $elaboro[1];
			
		//----------- firma3 ------------
		$firma3 = trim(odbc_result($cur,47));  
		$reviso = explode("»",$firma3);
		$reviso[0] = preg_replace('/[^A-Za-z0-9 *ÑÁÉÍÓÚáéíóú]/', '', $reviso[0]);
		$reviso[1] = strtr($elaboro[1], $sustituye);
		$reviso[1] = preg_replace('/[^A-Za-z0-9 *.ÑÁÉÍÓÚáéíóú]/', '', $reviso[1]);
		if (substr(utf8_decode($reviso[0]), -1) == "?") $n_reviso = substr($reviso[0],0,-1);
		else $n_reviso	= $reviso[0];
		if (substr(utf8_decode($reviso[1]), -1) == "?") $c_reviso = substr($reviso[1],0,-1);
		else $c_reviso = $reviso[1];

		//----------- firma4 ------------
		$firma4 = trim(odbc_result($cur,48));
		$aprobo = explode("»",$firma4);
		$aprobo[0] = strtr($aprobo[0], $sustituye);
		$aprobo[0] = preg_replace('/[^A-Za-z0-9 *.ÑÁÉÍÓÚáéíóú]/', '', $aprobo[0]);
		if (substr(utf8_decode($aprobo[0]), -1) == "?") $n_aprobo = substr($aprobo[0],0,-1);
		else $n_aprobo	= $aprobo[0];
		$pos = strpos(utf8_decode($aprobo[0]), "?");
		if ($pos > 0) $n_aprobo = utf8_encode($aprobo[0]);
		$c_aprobo = "FIRMA SOLICITANTE";

		//----------- firma6 ------------
		$firma6 = trim(odbc_result($cur,50));
		$jem = explode("»",$firma6);
		$jem[0] = preg_replace('/[^A-Za-z0-9 ]*.ÑÁÉÍÓÚáéíóú/', '', $jem[0]);
		if (substr($jem[0], -1) == "?") $n_jem = substr($jem[0],0, -2);
		else $n_jem = $jem[0];
		$jem[1] = preg_replace('/[^A-Za-z0-9 *.ÑÁÉÍÓÚáéíóú]/', '',  $jem[1]);
		if (substr($jem[1], -1) == "?") $c_jem = substr($jem[1],0,-2);
		else $c_jem = $jem[1];		

		//----------- firma7 ------------
		$firma7 = utf8_encode(trim(odbc_result($cur,51)));
		$cdo = explode("»",$firma7);
		$cdo[0] = preg_replace('/[^A-Za-z0-9 ]*.ÑÁÉÍÓÚáéíóú/', '', $cdo[0]);
		if (substr(utf8_decode($cdo[0]), -1) == "?") $n_cdo = substr($cdo[0],0,-1);
		else $n_cdo = $cdo[0];
		$cdo[1] = preg_replace('/[^A-Za-z0-9 ]*.ÑÁÉÍÓÚáéíóú/', '', $cdo[1]);
		if (substr(utf8_decode($cdo[1]), -1) == "?") $c_cdo = substr($cdo[1],0,-1);
		else $c_cdo = $cdo[1];
		...
		$pdf->Cell(190,4,'_____________________________________________',0,1,'C');
		...
		$pdf->SetXY($x+98,$y);
		$pdf->Multicell(92,4,utf8_decode($frm2),T,'C');
		$actual=$pdf->GetY();
		$pdf->Ln(2);
