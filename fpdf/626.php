...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$consulta_cb = "select * from cx_con_ban where unidad = '".$uni_usuario."' and periodo = '".$_GET['periodo']."' and ano = '".$_GET['ano']."' and cuenta = '".$_GET['cuenta']."'";
			$cur_cb = odbc_exec($conexion,$consulta_cb);
			$fecha_cb = substr(odbc_result($cur_cb,2),0,10);
			...
			if ($sigla1 <> "") if ($fecha_cb >= $fecha_os) $sigla = $sigla1;
	...
	if ($_GET['cuenta'] == '1') $nom_cta = 'GASTOS';
	else 
	{
		...
		$consulta_cta = "select * from cx_sal_cue where conse ='".$_GET['cuenta']."' and periodo = '".$_GET['periodo']."' and ano = '".$_GET['ano']."'";
	...
	$pdf->Cell(5,5,'',0,1,'C');
	$pdf->Cell(23,5,'FECHA',TR,0,'C');		
	$pdf->Cell(17,5,'No.',TR,1,'C');	
	...
	//$pdf->Cell(95,5,$pdf->Image($file,15,$actual-10,$w,30),0,0,'C');		
	...
	//$pdf->Cell(95,5,$pdf->Image($file,122,$actual-10,$w,30),0,1,'C');		
	...
	//$pdf->Cell(95,5,$pdf->Image($file,57,$actual-10,$w,30),0,1,'C');
	...
	$nom_pdf = "../fpdf/pdf/Conciliaciones/".$_GET['ano']."/Concilia_".$uni_usuario."_".$_GET['periodo']."_".$_GET['ano']."_".$_GET['cuenta'].".pdf";
