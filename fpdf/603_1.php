<?php
/* 603_1.php
   FO-JEMPP-CEDE2-603_1 - Lista de verificación.
   Directiva No. 0012 del 1 de Abril de 2025
 */ 
session_start();
require('../conf.php');
...
else
{
	...
	$des_tipo = $tipo == "0" ? "EXPEDIENTE DE RECOMPENSAS" : "EXPEDIENTE DE INFORMACIONES";
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$this->Cell(279,7,'LISTA DE VERIFICACIÓN DIRECTIVA No 0012 DEL 01 DE ABRIL DE 2025',0,1,'C');
			$this->Cell(279,7,$des_tipo,0,1,'C');
		...
		function morepagestable($datas, $lineheight=5)
		{
			...
			foreach($datas AS $row => $data)
			{
				...
				foreach($data AS $col => $txt)
				{
					...
					$this->SetXY($l,$h);
					if ($this->tablewidths[$col] > 141) $this->MultiCell($this->tablewidths[$col],$lineheight,$txt,0,'J');
	...
	$query = "";
	if($tipo == "0") {
		$query = "SELECT documentacion FROM dbo.lista_0012 WHERE es_recompensa = 1 and directiva='6' ORDER BY orden";
	} else {
		$query = "SELECT documentacion FROM dbo.lista_0012 WHERE es_informacion = 1 and directiva='6' ORDER BY orden";
	}
	  $sql = odbc_exec($conexion, $query);
	  $i = 0;
	  while($i<$row=odbc_fetch_array($sql))
	  {
		$items[] = odbc_result($sql,1);
	  }
	...
	$pdf->Cell(9,10,'NO.',0,0,'C');
	$pdf->Cell(225,10,'DOCUMENTACIÓN',1,0,'C');
	$pdf->Cell(10,10,'SI',1,0,'C');
	$pdf->Cell(10,10,'NO',1,0,'C');
	$pdf->Cell(24,10,'FOLIO(s)',1,1,'C');

	$actual=$pdf->GetY();
	$pl = 0;
	for ($f=0;$f<=count($items)-1;$f++)
	{	
		$actual=$pdf->GetY();
		$x = $pdf->Getx();
		$y = $pdf->Gety();	
		...
		$l1 = $lista[++$pl];
		$data[] = array($f+1, $items[$f], $si, $no, $l1);
		$pl++;

		control_salto_pag($pdf->GetY()); 
		$actual=$pdf->GetY();
		$pdf->tablewidths = array(10, 225, 10, 10, 24); 
