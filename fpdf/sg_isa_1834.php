...
Pruebas usar: http://192.168.1.107:8086/GASTOS/fpdf/1834.php?conse='2'&ano='2023'&ajuste='0'
              http://192.168.1.107:8086/GASTOS/fpdf/1834.php?conse=4&ano=2025&ajuste=0
*/
...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			$fecha_sa = substr(odbc_result($cur,2),0,10);
		...
		function morepagestable($datas, $lineheight=4)
		{
			...
			foreach($datas AS $row => $data)
			{
				...
				foreach($data AS $col => $txt)
				{
					...
					$this->SetFont('Arial','',8);
					$this->SetXY($l,$h);
					if ($this->tablewidths[$col] == 24 or $this->tablewidths[$col] == 30) $this->MultiCell($this->tablewidths[$col],$lineheight,$txt,0,'R');
	...
	function control_salto_pag($actual1)
	{
		global $pdf, $si;
		$actual1=$actual1+5;
		if ($actual1>=256.00125) $pdf->addpage();    //266.00125
		return $si = 1;
	} //control_salto_pag

	//Control cambio de sigla
	function verifica_sigla($v_sigla)
	{
		require('../conf.php');
	...
	$num_fir = explode("|",$fir);
	for ($i=0;$i<count($num_fir);++$i) $fir1[$i] = $num_fir[$i];
	...
	$pdf->Cell(39,5,'LUGAR Y FECHA:',0,0,'L');
	...
	for ($i=count($inter)-1;$i>=0;$i--)
	{
		...
		if (strlen($inter[$i]["cargo"]) >= 89) $lin = $lin + 3;
		else $lin = $lin + 2;
	}   //for
	$tt = "INTERVIENEN:";
	for ($i = 0; $i <= $lin-1; $i++) $tt = $tt."\n";
	$x = 9;
	$y = $pdf->Gety();
	$pdf->SetXY($x,$y);
	$pdf->Multicell(40,4,$tt,LB,'L');
	$pdf->SetXY($x+40,$y);
	$pdf->Multicell(152,4,$texto1_int,LB,'L');

	$x = 9;
	$y = $pdf->Gety();
	$pdf->SetXY($x,$y);
	$pdf->Multicell(40,4,"A S U N T O:",0,'L');
	$pdf->SetXY($x+40,$y);
	$pdf->Multicell(152,4,$asunto,L,'L');
	$actual=$pdf->GetY();
	$pdf->Ln(-3);
    ...
    while($rg<$row=odbc_fetch_array($cur1))
    {
		...
		$unidadsol = verifica_sigla(trim(odbc_result($cur2,7)));
		$unidadcen = verifica_sigla(trim(odbc_result($cur2,15)));
		...
		$texto_int = $texto_int;
		$data[] = array($radicado, $unidadsol, $unidadcen, $concepto, $texto_int, '$'.number_format($vsol,2));
	...
	$pdf->Cell(30,5,'$'.number_format($tvsol,2),1,1,'R');    //$tvaprobado   16/04/2025
	$pdf->SetFont('Arial','',8);
	...
	$anexos = "Anexo: Solicitud(es) de recursos adicionales relacionadas con sus soportes anexos.\n";
	$pdf->Ln(2);
	...
	control_salto_pag($pdf->GetY());
	$actual=$pdf->GetY();
	$cierre = "No siendo otro el objeto de la presente, se da por terminada y en constancia firman los funcionarios que intervinieron.";
	...
	for ($i=0;$i<$cmp;$i=$i+2)
	{
		...
		$pdf->SetXY($x+3,$y);
		$pdf->Multicell(87,4,$inter[$i]["nom"]."\n".$inter[$i]["cargo"],T,'C');
		$pdf->SetXY($x+90,$y);
		$pdf->Multicell(5,4,"",0,'C');
		$pdf->SetXY($x+95,$y);		
		$pdf->Multicell(87,4,$inter[$i+1]["nom"]."\n".$inter[$i+1]["cargo"],T,'C');
				control_salto_pag($pdf->GetY());
		if ($si == 1)
		{
			$x = $pdf->GetX();
	...
	$nom_pdf="pdf/InfAuto_".trim($sigla)."_".$_GET['conse']."_".$_GET['ano'].".pdf";
