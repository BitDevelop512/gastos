...
else
{
	...
	class PDF extends PDF_Rotate
	{
		function Header()
		{
			...
			if ($_GET['unidad'] == '999') $unidad_1 = $uni_usuario;
			else $unidad_1 = $_GET['unidad'];			

			//01/07/2023 - Se hace control del cambio de la sigla a partir de la fecha de cx_org_sub.
			$consulta1 = "select sigla, nombre, sigla1, nombre1, fecha from cx_org_sub where subdependencia = '".$unidad_1."'";
	...
	//01/07/2023 - Se hace control del cambio de la sigla a partir de la fecha de cx_org_sub.
	$consulta_sub = "select * from cx_org_sub where subdependencia = '".$unidad_1."'";
	...
	$consulta_reg = "select fecha from cx_rel_gas where unidad = '".$unidad_1."' and periodo = '".$_GET['periodo']."' and ano = '". $_GET['ano']."' order by fecha DESC";
	$cur_reg = odbc_exec($conexion,$consulta_reg);
	$fecha_rg = substr(trim(odbc_result($cur_reg,1)),0,10);
	if ($sigla1 <> "") if ($fecha_rg >= $fecha_os) $sigla = $sigla1;
    ...
    {
		$erogaciones[0][$i] = odbc_result($cur_ctrpag,1).". ".strtr(trim(odbc_result($cur_ctrpag,2)), $sustituye); //." (".odbc_result($cur_ctrpag,3).")";
