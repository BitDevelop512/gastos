...
/* 631.php
   ...
   (pág 234 - "Dirtectiva Permanente No. 00095 de 2017.PDF")
	...
	- 13/12/2023 - Generar el informe en MS Excel. Jorge Clavijo.
*/
...
else
{
	...
	$columna = array ('No', 'U.O.M', 'BR', 'UT', 'Unidad/Dep emp recurso','Fecha','Comprobante','No.','Valor Ingreso','Valor Egreso','Saldo','Concepto del gasto','Soporte','Recurso');
	...
	else
	{
		...
		$consulta_salc = "select * from cx_sal_cue where conse ='".$_GET['cuenta']."' and periodo = '".$periodo."'";
	...
	for ($ax=0;$ax<=count($data)-1;$ax++)
	{
		...
		$aax = $ax + 1;
		$data1[] = array($aax, $data[$ax][1], $data[$ax][2], $data[$ax][3], $data[$ax][4], $data[$ax][5], $data[$ax][6], $data[$ax][7], $data[$ax][8], $data[$ax][9], $saldo, $data[$ax][11], utf8_encode(trim($data[$ax][12])), $data[$ax][13]);		
	}   //for
	...
	foreach(range('B','O') as $cols) $objPHPExcel->getActiveSheet()->getColumnDimension($cols)->setAutoSize(true);
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, 3, 'SALDO ANTERIOR');
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, 3, $saldo_ant);

	$f_data1 = count($data1)+3;
	$ran3 = 'B4:O'.$f_data1;
	...
	$objPHPExcel->getActiveSheet()->getStyle($ran3)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_GENERAL);	
	$objPHPExcel->getActiveSheet()->getStyle('J3:l2000')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD_SIMPLE);

	for ($fil=0;$fil<=count($data1);$fil++)
	{
		for ($clm=2;$clm<=15;$clm++)
		{
			$cc = $clm - 2;
			$vlr = trim($data1[$fil][$cc]);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($clm-1, $fil+4, $vlr);
		}   //for
	}   //for
	...
	$fil = $fil + 3;
	$ran4 = 'B'.$fil.':O'.$fil;	
	$objPHPExcel->getActiveSheet()->getStyle($ran4)->applyFromArray(array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN,'color' => array('rgb' => 'FF000000')))));
	...
	$objPHPExcel->getActiveSheet()->mergeCells('M'.$fil.':O'.$fil);
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $fil, $saldo);

	//Graba archivo de Excel
	$nom_file = $sig_usuario."_".$_GET['periodo']."_".$_GET['ano']."_".$_GET['cuenta'].".xlsx";
