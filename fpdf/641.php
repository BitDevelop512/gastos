...
/* 641.php
   ...
   (pág 77 - "Dirtectiva Permanente No. 00095 de 2017.PDF")
	...
	25-02/2025 - Se agrega el campo recurso solictado por requerimiento - Jorge Clavijo.
	21/04/2025 - Se elimina la casilla de recurso solicitado por Jorge Clavijo.			
*/
	...
	else
	{
		...
		//25-02/2025 - Se agrega el campo recurso solictado por requerimiento - Jorge Clavijo.
		$recu = odbc_result($cur,58);
		$consulta14 = "select * from cx_ctr_rec where codigo = '".$recu."' and estado <> 'X'";
		$cur14 = odbc_exec($conexion,$consulta14);
		if (odbc_num_rows($cur14) == 0) $n_recu = "N/A";
		else $n_recu = odbc_result($cur14,2);
		...
		//----------- firma4 aprobó ------------
		$firma4 = trim($row0['firma4']); 
		if ($firma4 <> "") $firma4 = trim(odbc_result($cur,48));
		else
		{
			$consulta_fr = "select nombre, cargo from cx_usu_web where usuario = '".trim(odbc_result($cur,28))."'";
			$cur_fr = odbc_exec($conexion,$consulta_fr);			
			$firma4 = trim(odbc_result($cur_fr,1))."»".trim(odbc_result($cur_fr,2));
		}   //if
		...
		//21-04-2025 Se elimina la casilla de recurso solicitado por Jorge Clavijo.		
		//$pdf->Cell(15,5,'Recurso',L,0,'');
		//$pdf->Cell(57,5,$n_recu,L,1,'');
		$pdf->Cell(83,5,$tipo." -  ".$n_recu,L,1,'');
		$actual=$pdf->GetY();
		...
		if ($tipo1 == "1")
		{
			...
			while($i<$row=odbc_fetch_array($cur5))
			{
				...
				$consulta3 = "select * from cx_pla_gad where conse1='$conse' and ano='$ano' and interno='$i' order by conse";
		...
		{
			$ruta_local_tmp = $ruta_local."\\fpdf\\";
			$file = $ruta_local_tmp.basename(tempnam(getcwd(),'tmp'));
			$pdf->Output($file);
			$archivo = str_replace("\\", "", $file);
			echo "<HTML><SCRIPT>document.location='consulta1.php?f=$archivo';</SCRIPT></HTML>";
		}
	}
?>
