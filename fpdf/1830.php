...
else
{
	...
	if ($t_comite == 1)   //central
	{
		...
		// Ciudad de unidad
		$consulta_os = "select ciudad from cx_org_sub where subdependencia = '".$unidad."'";
...
/* 29/04/2025 - Jaime Morales - Tipo de registro de recompensa: 0 normal - 1 informacion */
		...
		if ($tipo == 1)
		{
			...
			if ($oficio == "0")
			{
				$consulta_rev1 = "select oficio, fec_ofi from cx_reg_rec where conse = '".$registro."' and ano = '".$ano1."'";
				$cur_rev1 = odbc_exec($conexion,$consulta_rev1);
				$oficio = trim(odbc_result($cur_rev1,1));
				$fec_ofi = trim(odbc_result($cur_rev1,2));
			}
		}
		else
		{
...
/* 02/08/2024 - Jaime Morales - Busqueda de Unidad Especial */
		$pregunta = "SELECT * FROM cv_unidades WHERE subdependencia='$unidad_rec'";
		$sql = odbc_exec($conexion,$pregunta);
		$var_uni = odbc_result($sql,1);
		$pregunta1 = "SELECT especial FROM cx_org_sub WHERE unidad='$var_uni' AND unic='1'";
		$sql1 = odbc_exec($conexion,$pregunta1);
		$var_esp = odbc_result($sql1,1);
		if ($var_esp == "0")
		{
		}
		else
		{
			$pregunta2 = "SELECT sigla FROM cx_org_sub WHERE unidad='$var_esp' AND unic='1'";
			$sql2 = odbc_exec($conexion,$pregunta2);
			$siguom = trim(odbc_result($sql2,1));
		}
/* Fin */
		...
		for ($r=0;$r<=$cn-1;$r++)
		{
			...
			$data1[] = array($elemento1, number_format($material1[1],0), '$'.number_format(trim($material1[3]),2), trim($material1[4],0), '$'.number_format(trim($material1[6]),2));
