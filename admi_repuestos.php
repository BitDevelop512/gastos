...
<div>
  <div id="soportes">
    <h3>Importar Repuestos Excel</h3>
    <div>
      <div id="load">
        <center>
...
if(isset($_POST['cargar']))
{
  ...
  if (move_uploaded_file($_FILES['archivo']['tmp_name'],"excel/" . $archivo1))
  {
    ...
    for ($row = 1; $row <= $lastRow; $row++)
    {
      ...
      {
        echo "<tr>";
        echo "<td width='60%'><font face='Verdana' size='2'>";
        $a = trim($worksheet->getCell('B'.$row)->getValue());                                 // Nombre Repuesto
        $a = iconv("UTF-8", "ISO-8859-1", $a);
        echo utf8_encode($a)."</font></td>";

        $b = trim($worksheet->getCell('C'.$row)->getValue());                                 // Unidad de Medida
        switch ($b)
        {
          case 'UNIDAD':
            $b1 = "1";
        ...
        if ($row > 1)
        {
          ...
          else
          {
            ...
            $sql1 = odbc_exec($conexion,"SELECT isnull(max(codigo),0)+1 AS conse FROM cx_ctr_rep");
            $consecu = odbc_result($sql1,1);
            $graba = "INSERT INTO cx_ctr_rep (codigo, nombre, medida, usuario, tipo, importa) VALUES ('$consecu', '$a', '$b1', '$usu_usuario', '$tipo', '$archivo2')";
            // Se graba log
            if (!odbc_exec($conexion, $graba))
            {
              $fec_log = date("d/m/Y H:i:s a");
              $file = fopen("log_importa_repuesto_err.txt", "a");
            ...
            {
              $fec_log = date("d/m/Y H:i:s a");
              $file = fopen("log_importa_repuesto_ok.txt", "a");
              fwrite($file, $fec_log." # ".$graba." # ".PHP_EOL);
              fclose($file);
            }
          }
