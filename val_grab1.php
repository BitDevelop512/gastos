...
if (is_ajax())
{
	...
	if ($adm_usuario == "10")
	{
		...
		$mensaje = "<br>SE HA REVISADO PLAN DE NECESIDADES UNIDAD CENTRALIZADORA, SE SOLICITA APROBACION.<br><br>";
	...
	if ($adm_usuario == "11")
	{
		$graba = "UPDATE cx_pla_cen SET ordena='$con_usuario', firma2='$nom_usuario', cargo2='$car_usuario' WHERE unidad='$uni_usuario' AND periodo='$periodo' AND ano='$ano'";
		odbc_exec($conexion, $graba);
		$conse = "1";
		// Se crea notificacion
		$cur1 = odbc_exec($conexion,"SELECT isnull(max(conse),0)+1 AS conse FROM cx_notifica");
		...
		$cur2 = odbc_exec($conexion,$query2);
		$usuario1 = odbc_result($cur2,1);
		$mensaje = "<br>SE HA ORDENADO PLAN DE NECESIDADES UNIDAD CENTRALIZADORA, SE SOLICITA VISTO BUENO.<br><br>";		
		$query2 = "INSERT INTO cx_notifica (conse, usuario, unidad, usuario1, unidad1, mensaje, tipo, visto) VALUES ('$consecu1', '$usu_usuario', '$uni_usuario', '$usuario1', '$uni_usuario', '$mensaje', 'S', '1')";
		$sql2 = odbc_exec($conexion, $query2);
	}
	// Si es Comandante Comando
	if ($adm_usuario == "12")
	{
		$graba = "UPDATE cx_pla_cen SET visto='$con_usuario', firma3='$nom_usuario', cargo3='$car_usuario' WHERE unidad='$uni_usuario' AND periodo='$periodo' AND ano='$ano'";
		odbc_exec($conexion, $graba);
		$conse = "1";
	}
