...
if (is_ajax())
{
	...
	if (($tipo == "1") or ($tipo == "2"))
	{
		...
		{
			$v_unidades = "";
			$query = "SELECT unidad, dependencia, unic FROM cv_unidades WHERE subdependencia='$uni_usuario'";
			$cur = odbc_exec($conexion, $query);
			$v_unidad = odbc_result($cur,1);
			$v_dependencia= odbc_result($cur,2);
			$v_unic = odbc_result($cur,3);
			if ($v_unic == "1")
			{
				$query1 = "SELECT subdependencia FROM cv_unidades WHERE unidad='$v_unidad' AND dependencia='$v_dependencia' ORDER BY subdependencia";
				$cur1 = odbc_exec($conexion, $query1);
				$i = 0;
				while ($i < $row = odbc_fetch_array($cur1))
	 			{
	 				$v_unidades .= odbc_result($cur1,1).",";
	 				$i++;
	 			}
	 			$v_unidades = substr($v_unidades,0,-1);
	 			$pregunta .= " AND unidad IN ($v_unidades)";
	 		}
	 		else
	 		{
...
// 18/02/2025 - Ajuste placas de todas las unidad de centralizadora
?>