...
if(isset($_POST['cargar']))
{
  ...
  if (move_uploaded_file($_FILES['archivo']['tmp_name'],"excel/" . $archivo1))
  {
    ...
    $tipo = $_POST['tipo'];
    // Se cambian de estado los registros anteriores
    $actu = "UPDATE cx_ctr_lla SET estado='X' WHERE estado='' AND tipo='$tipo'";
    ...
    for ($row = 1; $row <= $lastRow; $row++)
    {
      ...
      else
      {
        ...
        $a = trim($worksheet->getCell('B'.$row)->getValue());                                 // Marca
        $a = iconv("UTF-8", "ISO-8859-1", $a);
        echo utf8_encode($a)."</font></td>";

        echo "<td width='50%'><font face='Verdana' size='2'>";
        $b = trim($worksheet->getCell('C'.$row)->getValue());                                 // Descripcion
        ...
        $d = trim($worksheet->getCell('D'.$row)->getValue());                                 // Valor Unitario
        echo number_format($d, 2)."</font></td>";

        echo "<td width='10%' align='right'><font face='Verdana' size='2'>";
        $e = trim($worksheet->getCell('E'.$row)->getValue());                                 // IVA
        echo number_format($e, 2)."</font></td>";

        echo "<td width='10%' align='right'><font face='Verdana' size='2'>";
        $f = trim($worksheet->getCell('F'.$row)->getValue());                                 // Valor Total
        ...
        if ($row > 1)
        {
          ...
          else
          {
            ...
            $sql1 = odbc_exec($conexion,"SELECT isnull(max(codigo),0)+1 AS conse FROM cx_ctr_lla");
            $consecu = odbc_result($sql1,1);
            $graba = "INSERT INTO cx_ctr_lla (codigo, marca, descripcion, medida, tipo, valor, iva, total, usuario, importa) VALUES ('$consecu', '$a', '$b', '1', '$tipo', '$d', '$e', '$f', '$usu_usuario', '$archivo2')";
            // Se graba log
            if (!odbc_exec($conexion, $graba))
            {
              $fec_log = date("d/m/Y H:i:s a");
              $file = fopen("log_importa_llantas_err.txt", "a");
            ...
            {
              $fec_log = date("d/m/Y H:i:s a");
              $file = fopen("log_importa_llantas_ok.txt", "a");
              fwrite($file, $fec_log." # ".$graba." # ".PHP_EOL);
              fclose($file);
            }
          }
