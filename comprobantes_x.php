...
$objPHPExcel->getActiveSheet()->getStyle('A1:S1')->getFill()->getStartColor()->setARGB('CCCCCC');
$BStyle = array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)));
$objPHPExcel->getActiveSheet()->getStyle('A1:S1')->applyFromArray($BStyle);
...
for ($i=0;$i<count($paso1)-1;++$i)
{
    ...
    $v17 = $paso2[16];
    $v18 = $paso2[17];
    $objPHPExcel->getActiveSheet()->getStyle('I'.$j.':K'.$j)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $objPHPExcel->getActiveSheet()->getStyle('A'.$j.':S'.$j)->applyFromArray($BStyle);
    $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('A'.$j, $k)
        ->setCellValue('B'.$j, $v1)
        ->setCellValue('C'.$j, $v2)
    ...
    if (trim($v7) == "TOTALES")
    {
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A'.$m.':S'.$m)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A'.$m.':S'.$m)->getFill()->getStartColor()->setARGB('CCCCCC');
        $objPHPExcel->getActiveSheet()->getStyle('A'.$m.':S'.$m)->getFont()->setBold(true);
    }
    $j++;
}
