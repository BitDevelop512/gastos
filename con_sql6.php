  ...
  if (is_ajax())
  {
    if ($sup_usuario == "1")
    {
      ...
      if ($tipo == "3")
      {
        ...
        $salida->total = $total;
        $salida->actualiza = $actu;
      }
      // Comprobantes de ingreso
      if ($tipo == "4")
      {
        $pregunta = "SELECT ingreso, fecha, usuario, (SELECT sigla FROM cx_org_sub WHERE cx_org_sub.subdependencia=cx_com_ing.unidad) AS unidad, periodo, ano, estado, valor FROM cx_com_ing WHERE ingreso='$numero' AND unidad='$unidad' AND ano='$ano'";
        $sql = odbc_exec($conexion, $pregunta);
        $total = odbc_num_rows($sql);
        if ($total>0)
        {
          ...
          while ($i < $row = odbc_fetch_array($sql))
          {
            $salida->rows[$i]['ingreso'] = odbc_result($sql,1);
            $salida->rows[$i]['fecha'] = substr($row['fecha'],0,10);
            $salida->rows[$i]['hora'] = substr($row['fecha'],11,12);
            $salida->rows[$i]['usuario'] = trim(odbc_result($sql,3));
            $salida->rows[$i]['unidad'] = trim(odbc_result($sql,4));
            $salida->rows[$i]['periodo'] = odbc_result($sql,5);
            $salida->rows[$i]['ano'] = odbc_result($sql,6);
            $salida->rows[$i]['estado'] = trim(odbc_result($sql,7));
            $salida->rows[$i]['valor'] = odbc_result($sql,8);
            $i++;
          }
          $salida->total = $total;
        }
